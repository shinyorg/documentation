...
This is the heart of the API.  `store.Query<T>()` returns a fluent builder where you chain `.Where()`, `.OrderBy()`, `.Paginate()`, `.Select()` — then terminate with `.ToList()`, `.ToAsyncEnumerable()`, `.Count()`, `.Any()`, `.Remove()`, or any aggregate method.
