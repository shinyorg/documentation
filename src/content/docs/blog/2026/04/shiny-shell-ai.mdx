...
You can also seed route info into the system prompt upfront via the generated `AiRoutePrompt` string, so the model knows what's available without a discovery call.
