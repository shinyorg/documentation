...
That's it.  The service registers `IAiConversationService` with all the wiring — speech services from [Shiny.Speech](https://www.nuget.org/packages/Shiny.Speech), chat completions from [Microsoft.Extensions.AI](https://www.nuget.org/packages/Microsoft.Extensions.AI), audio playback, time provider, and optional message persistence.
...
The library targets plain `net10.0` — no MAUI dependency in the library itself.  [Shiny.Speech](https://www.nuget.org/packages/Shiny.Speech) handles the platform abstraction for speech and audio, so the same `IAiConversationService` works on:
