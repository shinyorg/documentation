...
That's it.  The service registers `IAiConversationService` with all the wiring — speech services from [Shiny.Speech](/speech/), chat completions from [Microsoft.Extensions.AI](https://www.nuget.org/packages/Microsoft.Extensions.AI), audio playback, time provider, and optional message persistence.
...
The library doesn't care which AI you use.  Implement `IChatClientProvider` to return any `IChatClient` from `Microsoft.Extensions.AI`:
...
- **ASP.NET** — Server-side chat without speech
