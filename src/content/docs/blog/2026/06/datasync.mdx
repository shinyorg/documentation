...
- **[Shiny Jobs](/client/jobs/)** runs *periodic background tasks* — WorkManager on Android, BGTaskScheduler on iOS, an in-process timer everywhere else.
- **[Shiny.Net.Http](/client/httptransfers/)** moves *files* in the background — `NSURLSession` on iOS, a foreground service on Android, a connectivity-driven `HttpClient` loop elsewhere.
...
| You need to… | Use | Why |
|---|---|---|
| Run a periodic background task (cleanup, refresh, telemetry flush) | **[Jobs](/client/jobs/)** | A scheduler with runtime criteria. No queue, no HTTP shape. |
| Move a large file up or down, resumable, in the background | **[HTTP Transfers](/client/httptransfers/)** | Range-aware resume for multi-megabyte blobs. One-way. |
| Reliably push record CRUD and pull deltas, offline-first | **[Data Sync](/client/datasync/)** | Persistent outbox + cursor inbox, drain-on-reconnect, conflict handling. |
...
- [Data Sync — Getting Started](/client/datasync/)
- [Architecture](/client/datasync/architecture) — why outbox + inbox, why platform-tiered transports
- [Platform Behavior](/client/datasync/platform-behavior) — what survives an app kill, and the Info.plist / manifest bits
- [Jobs](/client/jobs/) and [HTTP Transfers](/client/httptransfers/) — the two libraries it builds on
