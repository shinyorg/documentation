...
| Package | Description |
| --- | --- |
| <NugetBadge name="Shiny.Net.HttpServer" showLabel={true} /> | The server: protocols, routing, middleware, DI scopes, static files, WebSockets, SSE, sessions, OpenAPI, CORS, rate limiting, IP filtering, tunnelling — plus the compile-time typed-endpoint generator |
...
The typed-endpoint generator ships inside that package under `analyzers/` — it runs inside the
...
```csharp
server.MapGet("/api/users/{id:int}", async ctx => { /* … */ });
```
...
The other seven are clean, the four newest included — which is much of why gRPC and protobuf ask for
