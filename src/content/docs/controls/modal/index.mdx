...
- **Content template** — anything you like, scrolling in the body while the header and footer stay pinned, with `ContentMaxHeight` to say where that scrolling starts
...
A vetoed close nudges the panel (`NudgeOnBlockedDismiss`), so the refusal is visible rather than a dead click. Say *why* it was refused **from inside the panel** — anything the handler writes to the page is behind the modal that just refused to move.
:::
...
| `Width` / `Height` / `MaxWidth` / `MaxHeight` | `string?` | — | CSS. `Width` beats `Size`. `MaxHeight` caps the whole panel; unset, that is the viewport less the layer's gutter |
| `ContentMaxHeight` | `string?` | — | CSS. Caps the **scrolling region** rather than the panel — see below |
...
## Long content, and the footer that has to survive it

The panel is a column of header, body and footer in which only the body gives. `ScrollBody` — on by default — scrolls that body and leaves the header and footer exactly where they are, which is what keeps the buttons that answer the modal reachable however long the content runs.

There are two ways to say where the scrolling should start.

```razor
{/* Cap the panel: the body gets whatever is left after the header and footer take theirs. */}
<ModalView @bind-IsOpen="open" Title="Terms" MaxHeight="70vh" Buttons="@buttons">
    @* ... *@
</ModalView>

{/* Cap the content: the panel shrink-wraps up to 320px of body and scrolls past it. */}
<ModalView @bind-IsOpen="open" Title="Terms" ContentMaxHeight="320px" Buttons="@buttons">
    @* ... *@
</ModalView>
```

`MaxHeight` caps the whole panel, so the body ends up with that height less the chrome's. `ContentMaxHeight` caps the scrolling region itself: a short modal stays short and only starts scrolling at the cap, with no arithmetic over the header and footer to arrive at a panel height. Unset, the panel's cap is the viewport less the layer's gutter, which is why a long modal already scrolls without either being set.

A `ContentMaxHeight` scrolls whatever `ScrollBody` says, because a capped region that does not scroll only hides the rest.

`ScrollBody="false"` is the opposite trade: the panel is free to outgrow the viewport and the layer scrolls behind it, taking the header and footer with it. Use it when the content is the thing being read and the chrome is not needed on screen throughout — otherwise leave it on.
...
| `NudgeOnBlockedDismiss` | `true` | A refused dismissal shoves the panel, so it reads as "no" rather than as a dead click — a backdrop click that does not close, and any route `Closing` cancels |
