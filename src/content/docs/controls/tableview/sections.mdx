...
:::note
Section-level header/footer styling overrides global `TableView` header/footer styles. See [Styling](/controls/tableview/styling) for the full cascade.
:::
...
Setting `IsVisible="False"` hides the entire section including header, cells, and footer — and its
section separators, so the sections either side of it close up rather than leaving a doubled rule or a
stray line under the last visible section. Setting `FooterVisible="False"` hides only the footer while
keeping the rest visible.

`IsVisible` is live: flipping it re-renders the table, so binding it to a feature flag or a mode
switch is the intended use.

:::note
This is not the same as a **cell's** `IsVisible`, which the section reads when it renders and never
again. A cell whose visibility is only set after the section has been built may never appear at all —
prefer a section-level toggle, or give the cell a value that says "off" rather than removing it.
:::
