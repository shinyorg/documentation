...
:::note
When a property is set to `null` or `-1`, it falls back to the global value set on the parent `TableView`. This is the cascading style system — see [Styling](/controls/tableview/styling) for details.
:::
...
Same engine as [TextEntry](/controls/textentry/): `#` is a digit slot and every other character is a
literal inserted as the user types. `ValueText` holds the **raw digits** — so the value you bind and
validate is never the punctuation — while `FormattedValueText` holds what is on screen. The keyboard
and max length are set from the mask.
...
A bar docked to the top of the soft keyboard while the cell has focus — see
[Keyboard Accessory](/controls/textentry/keyboard-accessory/) for the full API and platform matrix.
On a settings form the useful shape is prev/next plus Done, with `FieldGroup` keeping the arrows inside
one section instead of walking every input on the page:
...
`TableView` is not virtualized, so unlike a `CollectionView` every cell on the page is realized and
prev/next can reach all of them.

:::note[What EntryCell does not borrow]
Only the input behaviour is shared. `TextEntry`'s chrome — tool slots, the floating label, hint text,
character count — is deliberately left out, because the cell already provides a title, description and
hint of its own.
:::

**Blazor** supports `Mask` and `IsAutoCompleteEnabled`. There is no accessory bar on Blazor — see
[Keyboard Accessory](/controls/textentry/keyboard-accessory/#why-there-is-no-blazor-version).
