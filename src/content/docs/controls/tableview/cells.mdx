...
| `UseHapticFeedback` | `bool` | `true` | Haptic click on cell tap |
...
Tapping the cell opens a prompt dialog for duration input. Accepts `H:MM` format (e.g., `1:30`) or plain minutes. The value is clamped to the configured min/max range.
