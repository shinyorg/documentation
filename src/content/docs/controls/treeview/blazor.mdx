...
The checkbox and the row are both hit targets — either toggles the row. Rows blocked by
`CanSelectSelector` render a disabled box. Set `ShowSelectionCheckBoxes="false"` for the older
highlight-only look. Changing `SelectionMode` clears the current selection.

Select or clear in bulk:

```csharp
await tree.SelectAllAsync();                    // everything selectable and loaded
await tree.SetBranchSelectedAsync(folder, true);// one item and its loaded descendants
await tree.DeselectAllAsync();                  // clears in any mode
```

Only nodes the tree has materialized can be checked — `await tree.ExpandAllAsync()` first to select a
lazy tree in full.
...
| `SelectionMode` | `BlazorTreeSelectionMode` | `Single` | `None` / `Single` / `Multiple`. Changing it clears the selection |
| `ShowSelectionCheckBoxes` | `bool` | `true` | Checkbox on every row while in `Multiple` mode |
...
| `EnableDragDrop` | `bool` | `false` | Enables native HTML5 drag/drop (via JS interop) with the `ItemDropped` event |
| `ItemDropped` | `EventCallback<TreeItemDroppedEventArgs<TItem>>` | — | Fires after a drop with `SourceItem` / `TargetItem` / `Position` |
...
tree.CollapseAll();
await tree.SelectAllAsync();     // Multiple mode only
await tree.DeselectAllAsync();
