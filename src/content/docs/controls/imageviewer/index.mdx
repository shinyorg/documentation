...
:::note
On Blazor, `Source` is a URL string instead of `ImageSource`. `IsOpen` uses `IsOpenChanged` callback for two-way binding. The `Aspect` property is not applicable on Blazor.
:::
