...
<img src="/images/colorpicker/blazor-s1.png" width="220" alt="Spectrum, hue, opacity and hex on Blazor" />
...
| `DoneText` | `string` | `Done` | Text of the popup's confirm button (MAUI) |

**Event:** `ColorChanged` (`EventHandler<Color>`)

## Committing and Cancelling

The popup writes `SelectedColor` on every drag so the button behind it previews the colour live. **Done** accepts whatever is showing; tapping the backdrop **discards** the change and puts back the colour the popup opened with, through the same property write a caller would make — so a two-way binding, `ColorChanged` and the button swatch all follow the revert.

:::note[MAUI only]
On Blazor a backdrop click still closes the popup keeping the current colour, and the confirm button's label is fixed. Keep your own copy of the colour before opening if you need a cancel there.
:::
...
- **Popup mode** -- `ColorPickerButton` opens the picker as an overlay dialog with a backdrop and Done button; on MAUI the backdrop cancels and `DoneText` labels the button
