...
| `BarcodeDetected` | `EventCallback<CameraBarcode>` | — | A barcode was decoded (`Format`, `Value`, normalized box) |
...
`StartAsync()` · `StopAsync()` · `CapturePhotoAsync()` → JPEG `byte[]` (filtered to match the preview) · `StartRecordingAsync(includeAudio = true)` · `StopRecordingAsync()` → WebM `byte[]` · `GetAvailableCamerasAsync()` → `IReadOnlyList<CameraDevice>` (`Id`, `Name`).
