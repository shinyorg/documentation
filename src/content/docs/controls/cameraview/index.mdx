...
The same filter is also baked into **captured photos** (`CapturePhotoAsync`) on Apple and Android, so a still matches the preview. **Recorded video is not filtered** (it records the raw feed), and Windows photos are unfiltered (it has no live filter).
