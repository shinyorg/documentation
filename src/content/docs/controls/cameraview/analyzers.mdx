...
#### Reading small, distant text

For OCR the scan window is not just an optimisation — it is the difference between finding small text and never seeing it. The platform engines **discard text below a minimum height** (Apple Vision's default is 1/32 of the image height, so ~34px in a 1080p frame) and **downscale the image before recognizing**, so a license plate through a windscreen or a sign across a car park never survives whole-frame OCR regardless of how you filter the results.

`OcrAnalyzer` therefore **crops to the scan window and recognizes the crop**, and adds two knobs for exactly this case:

| Property | Meaning |
|---|---|
| `MinimumTextHeight` | The floor, as a fraction of the recognized image's height. `0` = platform default. Apple only — MLKit and Windows expose no equivalent. |
| `MinimumInputHeight` | Upscale the crop to at least this many pixels tall before recognizing. `0` = off; only applies when a scan window is set. This is what buys small text back on Android. |
...
Writing your own analyzer? Call the shared recognizer directly. Boxes always come back in **full-frame** coordinates, never crop space:

```csharp
var text = await this.recognizer.RecognizeAsync(
    frame,
    new TextRecognitionOptions(
        RegionOfInterest:   region,      // normalized upright rect, null = whole frame
        MinimumTextHeight:  0.012f,
        MinimumInputHeight: 720),
    ct);
```

`TextRecognitionOptions` is the per-frame cache key, so two analyzers asking for the **same** region on one frame still cost a single OCR pass; one watching a different region gets its own, which is usually still cheaper than a whole-frame pass.

:::caution[Range is physics, not tuning]
Small-text OCR is bounded by how many pixels the characters occupy, and no threshold changes that. License plate characters are ~70mm tall, which at a typical phone focal length is around 20px at 5m and under 10px by 10m. Cropping and upscaling recovers the text the engine was throwing away; it cannot recover detail the sensor never resolved.
:::
...
There is one more package not covered here — `Shiny.Maui.Controls.Camera.Ai`, whose `AiDocumentAnalyzer<T>` detects that a document is present cheaply on every frame and then hands that one frame to a `Microsoft.Extensions.AI` `IChatClient` for structured extraction. Reach for it when the document is free-form or you'd rather not write parse rules; see [AI Document Scanner](/controls/cameraview/ai/).
...
var faces = new FaceAnalyzer { DetectLandmarks = true };   // off by default — costs extra per frame
faces.FacesDetected += (_, e) => Console.WriteLine($"{e.Faces.Count} face(s)");
camera.Analyzer = faces;
```

Apple Vision (iOS / Mac Catalyst / macOS), Android MLKit, and `Windows.Media.FaceAnalysis`. `FacesDetectedEventArgs.Faces` is a list of `DetectedFace` — `Bounds` (normalized upright), `Confidence`, optional `Landmarks`, and an optional `TrackingId` that stays stable across frames where the backend tracks, so a per-face effect can stay attached to the same person when several are in view.

**Landmarks are opt-in.** Set `DetectLandmarks = true` for anything that anchors to features — a `FaceMaskEffect` requires it. `FaceLandmarks` carries `LeftEye`, `RightEye`, `NoseBase`, `MouthLeft`, `MouthRight` and `MouthBottom` (all nullable — a face at a steep angle hides features from any backend) plus derived `EyeCenter`, `EyeDistance` (the stable scale reference for anything pinned to a face) and `Roll` in radians. Eyes are named **as seen on screen**, so the front camera's mirroring is already accounted for. Populated by Apple Vision and Android MLKit only; Windows and the managed fallback report boxes with `Landmarks == null`.

See [Face Masks](/controls/cameraview/face-masks/) for pinning an image to those landmarks.
...
An analyzer that holds something native — an ML Kit client, a Core ML model, an ONNX session — should create it lazily on first use and release it in `OnDetached()`, never in the constructor or a field initializer. `IFrameAnalyzer` has `OnAttached()` / `OnDetached()` hooks (default no-ops; `protected virtual` on `FrameAnalyzer`). The pipeline calls `OnDetached()` when the analyzer is removed or swapped, when `IsEnabled` goes false, or when the camera's handler disconnects — deferred until any in-flight `AnalyzeAsync` completes, and only once every camera sharing the analyzer has let go. `OnAttached()` fires when it becomes live again, so re-create on the next frame.

```csharp
protected override void OnDetached()
{
    this.session?.Dispose();
    this.session = null; // re-created lazily in AnalyzeAsync
}
```

The built-in Barcode, Face, OCR and document analyzers do exactly this; `BarcodeScanner` and `TextRecognizer` expose `ReleaseNativeResources()` if you use them directly.
