---
title: SheetView | Blazor Usage
description: Using SheetView in Blazor applications
---
...
:::note
On Blazor, colors are CSS strings. Two-way binding uses `IsOpen`/`IsOpenChanged` callback pattern. The `Location` property supports `Bottom` and `Top` (no `BottomTabs` on Blazor). Templates use Blazor `RenderFragment` instead of MAUI `DataTemplate`.
:::
