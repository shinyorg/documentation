...
Read-only state is on the instance: `CurrentState`, `Position`, `Duration`, `BufferedProgress`, `VideoWidth`, `VideoHeight`, `IsFullScreen`, `IsPictureInPictureActive`, `Capabilities`.

Events are parameters: `OnStateChanged`, `OnMediaOpened`, `OnVideoSizeChanged`, `OnMediaEnded`, `OnMediaFailed`, `OnPositionChanged`, `OnFullScreenChanged`, `OnPictureInPictureChanged`. `IsMuted` supports `@bind-IsMuted`.

`VideoWidth`/`VideoHeight` are the browser's `videoWidth`/`videoHeight` — the split-int form of the MAUI control's single `VideoSize`, and read the same way: through `OnVideoSizeChanged` rather than once at `OnMediaOpened`, since an adaptive source reports a new size mid-playback. See [Sizing the player to the video](/controls/mediaelement/#sizing-the-player-to-the-video).
