---
title: Ribbon
description: An Office-style tabbed command bar for .NET MAUI and Blazor — titled command groups, contextual tabs, split and menu buttons, a quick access row, a collapsing body, and groups that fold into buttons when the window narrows
---
...
It ships in the **core** package on both hosts, and targets everything they do:

- **`Shiny.Maui.Controls`** — MAUI, every target including iOS and Android, in the `Shiny.Maui.Controls.Ribbons` namespace.
- **`Shiny.Blazor.Controls`** — Blazor.
...
<PlatformSupport frameworks={["maui", "blazor"]} os={["windows", "macos", "linux", "ios", "android"]} />

<Aside type="caution" title="Moved out of the Desktop add-on">
The MAUI ribbon used to ship in `Shiny.Maui.Controls.Desktop`, whose target frameworks stop at the
desktop ones. That put it out of reach of every control in the core package that might want it — the
[Image Editor](/controls/imageeditor/) now does — because core cannot reference the add-on that
references it. The namespace moved with it, from `Shiny.Maui.Controls.Desktop.Ribbons` to
`Shiny.Maui.Controls.Ribbons`.

**XAML is unaffected.** The ribbon was always mapped onto the `http://shiny.net/maui/controls` URI, so
`shiny:Ribbon` reads exactly as it did. Only a C# `using` needs changing.
</Aside>
