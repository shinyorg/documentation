...
| `IsContentScrollEnabled` | `bool` | `true` | Wraps the content in a `ScrollView`. Set **`false`** when the content scrolls itself (a `TableView`/`CollectionView`) — nesting scroll-views collapses the inner one to near-zero height, leaving its rows blank. Safe with `ShowHeaderWhenClosed`: closed, only the header is on screen either way |
