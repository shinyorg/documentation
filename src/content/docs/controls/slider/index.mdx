---
title: Slider
description: A slider control with a cold-to-hot gradient track, blended thumb color, configurable tooltip, labelled stop points, custom thumb content, and a vertical orientation for .NET MAUI and Blazor
---
...
| Stop points | Vertical | Custom thumb |
|:---:|:---:|:---:|
| <img src="/images/slider/blazor-s2.png" width="280" alt="A slider snapping to labelled stop points on Blazor" /> | <img src="/images/slider/blazor-s3.png" width="280" alt="Two vertical sliders, one with labelled marks, on Blazor" /> | <img src="/images/slider/blazor-s4.png" width="280" alt="A thumb that carries the current value inside it on Blazor" /> |
...
- **Custom Thumb Content** -- `ThumbTemplate` puts an icon, a glyph or the value itself inside the thumb, with `ThumbWidth`/`ThumbHeight`/`ThumbCornerRadius` to shape the box around it.
...
<img src="/images/slider/s2.png" width="220" alt="Snapping marks, bubble labels and reference ticks on .NET MAUI" /> <img src="/images/slider/blazor-s2.png" width="360" alt="A slider snapping to labelled stop points on Blazor" />
...
<img src="/images/slider/s3.png" width="220" alt="A glyph thumb and two vertical sliders, one with marks, on .NET MAUI" /> <img src="/images/slider/blazor-s3.png" width="360" alt="Two vertical sliders, one with labelled marks, on Blazor" />
...
`ThumbTemplate` puts your own content inside the thumb. Its binding context (MAUI) / context
parameter (Blazor) is the slider's current `Value`.
...
The thumb does **not** grow to fit its content. The track's travel is measured from the thumb, so the
...
inside it. `ThumbCornerRadius` turns the default pill into any other shape.

On MAUI the template is realized once and rebound as the thumb moves, rather than rebuilt per frame,
so a template that animates keeps its state across a drag.
