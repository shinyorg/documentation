...
- **Tools Menu** -- (MAUI only) Set `ToolItems` with `FabMenuItem` instances to show a FabMenu on the left side of the input bar. The menu expands upward with tool actions (e.g., camera, image picker). When `ToolItems` is null or empty, the input bar renders normally.
