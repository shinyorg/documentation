...
| `StickyDayHeaders` | `bool` | `true` | Pin the current day group's header to the top while scrolling |
...
- **Sticky day headers** — The current day group's header pins to the top of the list while scrolling and swaps as the next group reaches it. The pinned header is input-transparent (scroll gestures pass through). Disable with `StickyDayHeaders="False"`. Changing `DayHeaderTemplate` or `EventItemTemplate` at runtime re-applies immediately
