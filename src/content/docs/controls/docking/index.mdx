...
- **`Shiny.Blazor.Controls`** — Blazor (browser today; Blazor Server / `BlazorWebView` once the contracts stabilize). There is **no Blazor add-on package**: docking ships in the main controls package, in the `Shiny.Blazor.Controls.Docking` namespace.
...
<img src="/images/docking/s1.png" width="560" alt="Solution Explorer, editor, Output and Properties panes docked on macOS AppKit" />

**Blazor**

| Default layout | Output panel docked to the bottom |
|:---:|:---:|
| <img src="/images/docking/blazor-s1.png" width="420" alt="Default layout on Blazor" /> | <img src="/images/docking/blazor-s2.png" width="420" alt="Output panel docked to the bottom on Blazor" /> |
...
> `AddShinyControls()` registers docking too, and takes the panels inline:
> `AddShinyControls(cfg => cfg.AddDockPanel<OutputPanel>("output"))`. Both are `TryAdd`, so
> calling either or both is safe.
...
### Panels that must not be closed

Some panels a surface simply cannot work without — a file explorer's folder tree, an editor's
document area. Closing one of those leaves a layout with nothing on screen that would bring it
back, because nothing reopens a panel unless you have built that yourself. Register those with
`canClose: false`:
...
The tab stops drawing a close button, and `HidePanelAsync` refuses the panel as well — so it is a
rule the host keeps rather than a button you happened to hide. Prefer it to suppressing
`.shiny-dock-tab__close` in CSS, which only removes the affordance and leaves every other close
path open.
