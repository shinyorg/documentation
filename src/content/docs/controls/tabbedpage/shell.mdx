...
<img src="/images/tabbedpage/s1.png" width="220" alt="The Shiny tab bar in place of a Shell's native one" />
...
The menu resolves from the most specific element outward: the **page**, then its **`ShellContent`**, then the **`Tab`**, then the **`ShellItem`**, and finally the centre button's own content. The first element that declares `MenuContent`/`MenuContentTemplate` supplies the content, and the first with any `Actions` rows supplies the rows — a page's rows **replace** the `ShellContent`'s, they never merge. Rows declared on a `ShellContent` bind against its `BindingContext` (the Shell's, unless you set one), and the button follows edits to any of those collections and every tab switch live.
