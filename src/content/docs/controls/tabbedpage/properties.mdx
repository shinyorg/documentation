...
`TabBarIsVisible`, `ContentBehindTabBar`, `CenterButton`, `IndicatorStyle`, `LabelMode`, `SelectedColor`, `UnselectedColor`, `IndicatorColor`, `BarStyle`, `BarMaterial`, `BarGlassTint`, `BarHeight`, `BarBackgroundColor`, `BarCornerRadius`, `BarMargin`, `HasShadow`, `IconSize`, `AnimateIcons`, `BarBackgroundOpacity`, `MaxVisibleTabs`, `MinTabWidth`, `OverflowTitle`, `OverflowIcon` — each forwards to the same property on `TabBar`, which is exposed read-only for everything else.
...
| `BarBackgroundOpacity` | `double` | `1` (background only — see below) |
| `BarMaterial` | `TabBarMaterial` | `Solid` (glass is iOS 26 — see below) |
| `BarGlassTint` | `Color?` | `null` (the system's own) |
| `MaxVisibleTabs` | `int` | `0` (auto — from the bar's width) |
| `MinTabWidth` | `double` | `72` |
| `OverflowTitle` | `string` | `"More"` |
| `OverflowIcon` | `string?` | `"more"` |
| `PageContext` | `BindableObject?` | `null` |
| `IsMenuOpen` | `bool` | `false` (two-way) |
| `SelectionChangedCommand` | `ICommand?` | `null` |

:::note[`BarBackgroundOpacity` fades the background, not the bar]
Icons, labels, badges and the indicator stay fully opaque — a bar whose tabs fade along with it is
unreadable well before the background gets interesting. The alpha goes on the colour rather than on a
view, because `Opacity` on the surface takes everything inside it down too and opacity multiplies
down the tree, so a child cannot undo it. It multiplies into any alpha the colour already carried and
follows a theme swap. Pair it with `ContentBehindTabBar` — or `BarStyle="Floating"`, which implies it
— or there is nothing behind the glass to see.
:::

:::note[`BarMaterial` is honoured on iOS 26 and ignored everywhere else]
`Glass` and `GlassClear` put a real `UIGlassEffect` behind the bar. On any other head — an older iOS,
Android, Windows, GTK4, Mac Catalyst, macOS AppKit — the value is remembered and the bar keeps
painting its fill, so the same XAML runs everywhere. Where glass does apply it replaces the
background outright: `BarBackgroundColor` and `BarBackgroundOpacity` are not painted, `HasShadow` is
ignored, and the content runs under the bar as it does for `BarStyle="Floating"`. Colour it with
`BarGlassTint`.
:::

`TabBarMaterial`: `Solid` (default) · `Glass` · `GlassClear`.
...
actually on. The overflow tab carries `tab-more`. A tab with neither a `Route` nor a `Title` gets none, rather than an index-based id
that would point at a different tab as soon as the order changed.

Methods: `GoTo(int)`, `GoTo(string)`, `OpenMenu()`, `CloseMenu()`, `OpenOverflow()`, `SelectOverflowItem(item)`.

Read-only: `HasOverflow`, `OverflowItems`, `IsOverflowCellSelected`.
