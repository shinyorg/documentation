...
| Member | Description |
|---|---|
| `SetIcon(Func<Stream>)` | Set the icon from a stream factory. PNG or ICO bytes both work — Windows auto-wraps PNG as ICO, and macOS scales whatever you hand it to the menu bar's own height, so an ordinary full-size app icon is a fine source |
