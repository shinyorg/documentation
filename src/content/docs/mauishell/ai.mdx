...
:::caution
AI extensions are **disabled by default**. You must explicitly opt in via MSBuild and install the `Microsoft.Extensions.AI` NuGet package. If AI extensions are enabled without the package, the source generator emits a **SHINY003** compiler error.
:::
...
2. Enable AI extensions in your `.csproj`:
```xml
<PropertyGroup>
...
:::note
When AI extensions are disabled, `GetGeneratedRouteInfo` is still generated — only the AI-specific methods are omitted. See [Source Generation](/mauishell/sourcegen/#configuring-source-generation) for all available MSBuild properties.
:::
