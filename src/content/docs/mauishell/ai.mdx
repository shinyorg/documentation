...
public partial class WorkOrderViewModel(INavigator navigator) : ObservableObject, IQueryAttributable
{
    [ShellProperty("Summarize what is broken based on what the user said", required: true)]
    public string Description { get; set; } = string.Empty;
...
AI-friendly navigation that accepts `Dictionary<string, string>` and dispatches to `NavigateTo<TViewModel>` with direct property setters. Returns a confirmation message string.
