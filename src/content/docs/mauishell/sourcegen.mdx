...
The source generators produce up to six files (nav extensions can be individually disabled):
...
Fully inferred `Show{Route}Dialog` extension methods on `INavigator`, emitted only for `[ShellMap]` ViewModels that also implement [`IDialogAware<T>`](/mauishell/dialogs#viewmodel-dialogs). Not generated at all when there are none.
...
These wrappers exist because C# cannot infer a type argument from a constraint — calling `INavigator.ShowDialog<TViewModel, T>` directly always requires *both* type arguments spelled out. The generated method closes both of them and surfaces `[ShellProperty]` values as parameters, in the same required-then-optional order as `NavigateTo{Route}`.

Dialog methods are deliberately excluded from the AI tool surface — an AI agent should be driving navigation, not blocking on a modal awaiting human input.
...
<PropertyGroup>
    ...
    <!-- Disable navigation extensions (NavigationExtensions.g.cs, NavigationBuilderNavExtensions.g.cs, DialogExtensions.g.cs, and AddGeneratedMaps) -->
...
| `ShinyMauiShell_GenerateNavExtensions` | `true` | `NavigationExtensions.g.cs`, `NavigationBuilderNavExtensions.g.cs`, `DialogExtensions.g.cs`, `AiExtensions.g.cs`, and `NavigationBuilderExtensions.g.cs` (`AddGeneratedMaps`) |
...
`DialogExtensions.g.cs` is additionally only generated when at least one of those ViewModels implements `IDialogAware<T>`.
