...
| `ShinyMauiShell_GenerateNavExtensions` | `true` | `NavigationExtensions.g.cs`, `NavigationBuilderNavExtensions.g.cs`, and `NavigationBuilderExtensions.g.cs` (`AddGeneratedMaps`) |

`NavigationBuilderExtensions.g.cs` (`AddGeneratedMaps()`) is only generated when at least one `[ShellMap]` attribute is detected and `ShinyMauiShell_GenerateNavExtensions` is enabled. If maps are detected but nav extensions are disabled, the generator emits a **SHINY002** warning.
