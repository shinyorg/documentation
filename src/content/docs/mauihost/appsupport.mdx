...
On a macOS (AppKit) head this also calls `AddMacOSEssentials()` for you. On a Linux (GTK4) head call
`AddLinuxAppSupport()` from `Shiny.Extensions.MauiHosting.Linux` instead — see [Desktop Backends](/mauihost/desktop).
...
| `Platform` | `DeviceInfo.Platform.ToString()` — `"Android"`, `"iOS"`, `"WinUI"`, `"macOS"` |
| `DeviceIdiom` | `DeviceInfo.Idiom` — `Phone`, `Tablet`, `Desktop`, `TV`, `Watch` |
| `PlatformVersion` | `DeviceInfo.Version` — the OS version |
| `CurrentOrientation` + `OrientationChanged` | Surfaced through MAUI's `DeviceDisplay.MainDisplayInfoChanged` on iOS/Android/macCatalyst/macOS/Windows |
