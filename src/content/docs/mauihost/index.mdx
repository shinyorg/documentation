...
MAUI apps often end up with a massive `MauiProgram.cs` where every plugin, service, and lifecycle hook is crammed together. Shiny MAUI Hosting lets you break that apart into modular `IMauiModule` classes — each responsible for its own service registration and post-build initialization. It also provides a static `Host.Services` accessor, platform lifecycle dispatch, an `IAppSupport` service for orientation/culture/time-zone change detection and device info, and an `IAppStore` service for cross-platform store version lookups and deep links.
...
- Opt-in registration: each capability ships as its own extension method (`AddPlatformLifecycle`, `AddAppSupport`, `AddAppStore`) so apps only pay for what they use
...
3. Wire everything up in `MauiProgram.cs`:
   ...
   builder
       ...
       .AddPlatformLifecycle()                     // lifecycle hooks (iOS/Android/macCatalyst)
...
Call `AddPlatformLifecycle()` to register the `ILifecycleExecutor` and wire MAUI's `ConfigureLifecycleEvents`. Register services that implement lifecycle interfaces — the executor dispatches events to all registered handlers automatically.
