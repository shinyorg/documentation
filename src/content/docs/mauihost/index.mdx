...
- `IStartupService` — install or remove the app from the desktop OS "launch at login" list on Windows, macOS (Mac Catalyst and AppKit), and Linux
- Opt-in registration: each capability ships as its own extension method (`AddAppSupport`, `AddAppStore`, `AddStartupService`) so apps only pay for what they use
...
3. Wire everything up in `MauiProgram.cs`:
   ...
   builder
       ...
       .AddStartupService()                        // IStartupService (omit if not needed)
...
`IStartupService` installs the app into the desktop operating system's startup ("launch at login") list — the Windows `Run` key, macOS login items via `SMAppService`, or a Linux XDG autostart entry. See [Startup Service](/mauihost/startup) for states, options, and per-platform behavior.
