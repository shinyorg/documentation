...
3. Register with the **Linux** extension methods:
   ...
   builder
       ...
       .AddStartupService();                          // XDG autostart (or a systemd unit via LinuxMode), from the base package
...
| `IStartupService` | The base package's `~/.config/autostart/{Identifier}.desktop` entry by default, or a systemd user/system unit via `LinuxMode` — see [Startup Service](/mauihost/startup/). No Linux-specific registration needed |
