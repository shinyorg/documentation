...
Local notifications are **not supported in Blazor WebAssembly**. The browser's `Notification` API exists, but it has very different semantics (no scheduling, no channels, no geofence triggers, dependent on the page being loaded) and the WASM implementation in `Shiny.Notifications` was removed. Use [Push Notifications](/push) (web push) for notifying users when your Blazor app isn't open.
:::
