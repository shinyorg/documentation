...
GPS-driven geofence monitoring for iOS and Android. Built on [Shiny.Locations](/client/locations/gps) for background GPS and [Shiny.Spatial](/spatial/) for spatial queries.
...
:::note[Regions that aren't a bundled SQLite file]
This monitor reads regions from `Shiny.Spatial` tables in a local `.db` you ship with the app. If your
regions are application data — managed server-side, edited by users, synced, or sitting alongside the rest
of your documents — [DocumentDb geofencing](/documentdb/geofencing) does the same job over
[Shiny.DocumentDb](/documentdb/) documents instead, and runs against a shared server database as readily as
an on-device SQLite file.
:::
...
:::tip
The [pre-built databases](/spatial/prebuilt) (US states, US cities, Canadian provinces, Canadian cities) work directly with the geofencing monitor. Bundle them as MAUI raw assets and copy on startup.
:::
...
| `MaximumDistance` | _(unset)_ | Safety-net cap — a reading is always processed once the device has moved this far, even if `MinimumTime` hasn't elapsed |
| `MaximumTime` | _(unset)_ | Safety-net cap — a reading is always processed after this much time has passed, even if `MinimumDistance` hasn't been met |
| `Entries` | Empty | List of database/table pairs to monitor |

Minimum thresholds use **AND** logic — when both are set, the device must move `MinimumDistance` *and* `MinimumTime` must elapse before a reading is processed. Maximum thresholds use **OR** logic and override the minimums: crossing either `MaximumDistance` or `MaximumTime` forces the reading through. Leave the maximums unset to disable the safety net (the default).
...
The spatial query uses the [two-pass pipeline](/spatial/) — the R*Tree index eliminates 99%+ of candidates before any geometry computation, keeping each GPS reading efficient even against large databases.
