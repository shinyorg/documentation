...
:::tip[Need richer queries?]
These queries cover distance, envelope, and intersection against a local SQLite file. If you need the full
topological predicate set (`GeoContains`, `GeoTouches`, `GeoCrosses`, `GeoOverlaps`, …), nearest-neighbour
search, or the same geospatial queries running against a server database, use
[Shiny.DocumentDb's spatial support](/documentdb/spatial) — it works over SQLite, PostgreSQL, MySQL,
SQL Server, Oracle, DuckDB, CosmosDB, and MongoDB.
:::
...
:::tip[Coordinate or Point — either works]
The geometry-based methods (`Intersecting`, `ContainedBy`, and the `Find*` equivalents) take a
`Geometry`, while the distance methods (`WithinDistance`, `OrderByDistance`) take a `Coordinate`.
A `Coordinate` implicitly converts to a `Point`, so you can pass a bare coordinate to the geometry
methods too — no need to wrap it in `new Point(...)` first when all you have is a lon/lat.

```csharp
var here = new Coordinate(-104.99, 39.74);
var results = table.Query()
    .Intersecting(here)   // Coordinate flows in as a Point
    .ToList();
```
:::
