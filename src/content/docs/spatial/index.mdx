...
:::tip[Need more than a local SQLite file?]
Shiny.Spatial is deliberately small — an on-device, file-based spatial store with distance, envelope, and
intersection queries. If your needs are bigger, use [Shiny.DocumentDb's spatial support](/documentdb/spatial)
instead. It gives you the full topological predicate family (`GeoIntersects`, `GeoContains`, `GeoTouches`,
`GeoCrosses`, `GeoOverlaps`, …), `NearestNeighbors`, LINQ composition, and distance ordering — over the
documents you're already storing, and against whichever database you're already using: SQLite on device plus
PostgreSQL, MySQL, SQL Server, Oracle, DuckDB, CosmosDB, and MongoDB on the server.
:::

| | |
|---------|---------|
| GitHub | <GitHubStars org="shinyorg" repo="geospatialdb" /> |
| Downloads | <NugetDownloads name="Shiny.Spatial" /> |

<PlatformSupport frameworks={["dotnet"]} os={["android", "ios", "windows"]} />
...
<AiSkill skillName="shiny-spatial" />
