...
Tag the provider with `[Singleton]` from `Shiny.Extensions.DependencyInjection` (or register it manually for every interface) and call `AddGeneratedServices()`:

```csharp
services.AddPush<MyPushDelegate>();
services.AddGeneratedServices(); // emits the registration for [Singleton] MyPushProvider
```

If you prefer explicit registration without the source generator:

```csharp
services.AddSingleton<MyPushProvider>();
services.AddSingleton<IPushProvider>(sp => sp.GetRequiredService<MyPushProvider>());
```
