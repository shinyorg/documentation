...
| Downloads | <NugetDownloads name="Shiny.Push" /> |
| Blazor | <NugetDownloads name="Shiny.Push.Blazor" /> |

<PlatformSupport frameworks={["dotnet", "maui", "blazor"]} os={["android", "ios", "tvos", "macos", "windows"]} />
...
| tvOS | APNs (silent / badge only) | `Shiny.Push` |
...
:::caution[tvOS]
tvOS registers with APNs and receives **silent (background) pushes** exactly as iOS does — `IPushDelegate.OnReceived` fires and your token handling is unchanged. What tvOS does not have is a notification a user can see or tap:

- `UNNotificationContent` on tvOS carries nothing but a badge count — no title, no body, no user info.
- There is no `UNNotificationResponse`, so **`IPushDelegate.OnEntry` is never raised** and nothing is presented in the foreground.
- `RequestAccess()` asks for `UNAuthorizationOptions.Badge` alone rather than Alert + Badge + Sound, because the other two do nothing there.

Design tvOS pushes as a signal to go fetch, not as a message to read.
:::
