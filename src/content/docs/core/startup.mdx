...
:::tip
If your startup task also needs to react to the app entering the foreground or background, inherit from `ShinyLifecycleTask` instead. It implements `IShinyStartupTask` **and** the platform `IApplicationLifecycle` interfaces in one base class — see [Lifecycle Hooks](/core/lifecycle).
:::
