...
## Checking Availability & Capability

Before reading or writing, gate on two things:

- **`IsAvailable`** — a whole-store check. On iOS this is always `true`; on Android it's `true` only when Health Connect is installed and the SDK status is OK.
- **`IsSupported(DataType, PermissionType)`** — a *per-metric* check. It returns `false` (rather than throwing) when the store is unavailable, the value is not a defined `DataType`, or the type/access is unsupported on the current platform. The `permission` argument defaults to `PermissionType.Read`; pass `Write` or `ReadWrite` to check write support.

```csharp
if (!health.IsAvailable)
    return; // Health Connect not installed on Android, etc.

if (health.IsSupported(DataType.StepCount))
{
    var steps = await health.GetStepCounts(start, end, Interval.Days);
}

// check write capability for a specific metric
if (health.IsSupported(DataType.Weight, PermissionType.Write))
{
    await health.Write(new NumericHealthResult(DataType.Weight, DateTimeOffset.Now, DateTimeOffset.Now, 75.0));
}
```

Prefer `IsSupported` over wrapping reads/writes in `try`/`catch` — it's the same capability seam that read-only or partial-coverage backends use to report what they support.
...
1. **Gate on availability and capability** — Check `IsAvailable` and `IsSupported(dataType, permission)` before reading; they return `false` instead of throwing for unavailable stores or unsupported metrics
2. **Always request permissions first** — Call `RequestPermissions` before reading data
