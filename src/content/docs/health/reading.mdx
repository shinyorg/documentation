...
Before reading or writing, gate on **`IsAvailable`** — a whole-store check. On iOS this is always `true`; on Android it's `true` only when Health Connect is installed and the SDK status is OK.
...
public class HealthDashboardViewModel(IHealthService health)
{
    async Task LoadDataAsync()
    {
        ...
        // 2. Check which permissions were granted - the user can grant a subset, so only
        //    query the types that came back true
...
:::caution[A partial grant is the normal case]
Both platforms let the user approve some of what you asked for and refuse the rest, so never treat
`RequestPermissions` as all-or-nothing. On Android, querying a `DataType` whose permission was not
granted **throws** a `HealthConnectException` wrapping a `SecurityException` — it does not return an
empty list — so one unapproved type will abort a whole dashboard load if you query unconditionally.
Gate each query on the per-`DataType` result, or wrap them individually.

On iOS, `Success` is not a reliable signal: HealthKit deliberately does not reveal read denials, so
it can report `true` and still return no samples.
:::
...
1. **Gate on availability** — Check `IsAvailable` before reading; it returns `false` for unavailable stores (e.g. Health Connect not installed on Android)
