...
Commands are much like [Requests](./requests) but without the result.  Commands allow you to do things that Requests simply
...
public class MyRequestMiddleware : IRequestMiddleware<ResponseRequest, Response>
{
    public async Task<Response> Process(
        RequestContext<ResponseRequest> context,
...
```csharp
public class MyCommandMiddleware<TCommand>: IRequestMiddleware<TCommand> where TCommand : ICommand
{
