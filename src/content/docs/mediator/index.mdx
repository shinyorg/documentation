...
hardly any effort whatsoever.  Checkout our [Getting Started](./getting-started) guide to see how easy it is.  Imagine using 1 line of code to add offline, caching, or validation to your code!
...
- [Request/Response Handling](./requests)
- [Event Publication](./events)
- [Async Enumerable Stream Requests](./streams)
...
- Easy to Unit Test
- Checkout our [MAUI](./extensions/maui) & [Blazor](./extensions/blazor) 
...
- Save the Boilerplate + Receive the Power of Middleware
    - [Dapper Extension](./extensions/dapper) for Easy Query Handling
    - [HTTP Extension](./extensions/http) for Easy API handling - OpenAPI Contract Generation takes it even one step further
    - Map contracts directly to handlers with our [ASP.NET Extension](./extensions/aspnet)
- [Epic Out-of-the-Box Middleware](./middleware/)
    - [Offline Data](./middleware/offline)
    - [Caching](./middleware/caching)
    - [Resiliency](./middleware/resilience)
    - [User Exception Handling notifications](./middleware/usererrornotifications)
    - Exception Handling logging
    - [Performance Time Logging](./middleware/performancelogging)
    - [Main Thread Dispatching](./middleware/mainthread)
    - [Replayable Streams](./middleware/replay)
    - [Refresh Timer Streams](./middleware/refresh)
    - [Command Scheduling](./middleware/scheduling)
...
public class GetDataRequestHandler : IRequestHandler<GetDataRequest, MyData> {
    ...
    public async Task<MyData> Handle(GetDataRequest request, MediatorContext context, CancellationToken cancellationToken) {
        // ...
    }
}
```
...
{
    public async Task Handle(MyPageNavigatonRequest request, MediatorContext context, CancellationToken cancellationToken)
    {
