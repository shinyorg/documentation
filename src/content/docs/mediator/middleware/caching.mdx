...
2. Create your request handler - you can mark Cache here with the attribute or you can use [Configuration](./).
   ...
   public partial class MyHandler : IRequestHandler<MyRequest, MyResult>
   {
       ...
       public async Task<MyResult> Handle(MyRequest request, IMediatorContext context context, CancellationToken ct)
       {
           return new MyResult();
       }
   }
   ...
   :::note
   Using attributes like [Cache] requires you mark the class as partial for our source generator to work correctly.
   :::
...
:::caution
Persistent cache is stored using JSON serialization (out of the box).  If AOT & Trimming is something you want in your app.  You need to either:

1. Implement your own `Shiny.Mediator.Infrastructure.ISerializerService` to deal with your own serialization needs
2. Use System.Text.Json source generators and ensure the ISerializerService has the JsonContext passed to it.
3. Use our builtin JSON converter source generator.  Simply make your result type partial and set the attribute `[SourceGenerateJsonConverterAttribute]` on the type.
:::
