...
:::note
Both `IConfiguration` and `ILogger` are **optional** to the mediator.  Every built-in service and middleware takes them as
optional constructor arguments, so you don't need to call `AddLogging()` or register an `IConfiguration` just to get the
mediator running.

When there is no `IConfiguration` in the container, any configuration-driven behaviour (`Cache`, `Offline`, `ReplayStream`,
`Resilience`, `TimerRefresh`, `PerformanceLogging`, `UserErrorNotifications`) simply resolves to "not configured" and the
middleware passes the call straight through — the attribute-based equivalents (eg. `[Cache]`, `[OfflineAvailable]`,
`[TimerRefresh]`) keep working.  When there is no logging, log statements are skipped.
:::
...
### Ordering Your Middleware

By default, middleware executes in DI registration order.  You can explicitly control execution order using `[MiddlewareOrder]` on your middleware class.  Lower values run first (outermost in the pipeline).

```csharp
[MiddlewareOrder(-100)]  // runs first
[MediatorSingleton]
public class ValidationMiddleware<TRequest, TResult> : IRequestMiddleware<TRequest, TResult>
    where TRequest : IRequest<TResult>
{ ... }

[MiddlewareOrder(100)]  // runs closest to the handler
[MediatorSingleton]
...
Middleware without `[MiddlewareOrder]` defaults to order `0`.  Middleware with the same order preserves DI registration order.

See [Middleware Ordering](/mediator/middleware/ordering) for full details.
