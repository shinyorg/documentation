...
3. With any request handler that provides a result, you can now add the following attribute
    ...
    ```csharp
    public partial class MyRequestHandler : IRequestHandler<MyResponse, MyResponse>
    {
        [Resilient("test")]
        public async Task<MyResponse> Handle(MyResponse request, IMediatorContext context, CancellationToken cancelToken)
        {
            // do your work here
