...
:::note
`[MiddlewareOrder]` is a class-level attribute placed on the **middleware class**, not on handler methods.  This is different from handler-level attributes like `[Cache]`, `[Sample]`, or `[Throttle]`.
:::
