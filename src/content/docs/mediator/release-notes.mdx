...
<RN type="fix">Fixed `StorageCacheService.GetOrCreate` race condition — concurrent calls could invoke the factory multiple times. Now uses `SemaphoreSlim` with double-check locking to ensure the factory runs exactly once</RN>
...
<RN type="fix">Fixed `AbstractFileStorageService` concurrent access — simplified semaphore patterns and prevented potential deadlocks from double semaphore acquisition in `GetFileIndexer`</RN>
