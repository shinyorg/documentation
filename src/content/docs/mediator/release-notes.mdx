...
<RN type="fix">**Generated JSON converters now handle nullable enum properties under NativeAOT** — when `MediatorHttp` items use `GenerateJsonConverters="true"`, the `JsonConverterSourceGenerator` previously detected only `TypeKind.Enum` and missed `Nullable<TEnum>` (which has `TypeKind.Struct`). Nullable enum properties fell through to `JsonSerializer.Deserialize<TEnum?>(ref reader, options)` and `JsonSerializer.Serialize(writer, value, options)`. At runtime under NativeAOT this triggers `NullableConverter<TEnum>`, which uses reflection to look up the inner enum's converter and throws `'System.Text.Json.Serialization.Converters.NullableConverter\`1[YourEnum]' is missing native code or metadata`. The generator now unwraps `Nullable<TEnum>` and emits `(TEnum?)Enum.Parse<TEnum>(reader.GetString()!)` on read and `value.Prop?.ToString()` on write — fully inline and trim/AOT-safe</RN>
