...
3. Now your viewmodel (or page) can simply implement the IEventHandler interface to participate
    ...
    {
        public async Task Handle(TestEvent @event, MediatorContext context, CancellationToken ct)
        {
        }
        
        public async Task Handle(TestEvent2 @event, MediatorContext context, CancellationToken ct)
        {
        }
    }
    ```
