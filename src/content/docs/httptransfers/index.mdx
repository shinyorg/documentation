...
<PlatformSupport os={["android", "ios", "tvos", "windows", "macos", "linux", "web"]} />
...
| tvOS | `Shiny.Net.Http` | `NSURLSession` background session — same as iOS |
...
:::caution[tvOS storage]
tvOS uses the same background `NSURLSession` iOS does, so transfers continue while the app is suspended. What differs is storage: an Apple TV has no user-visible file system and a small, evictable app container. Write transfer output to the cache directory, treat it as something the OS may reclaim between launches, and do not design around a download surviving indefinitely.
:::
...
```csharp
// iOS / Android / Windows (platform-aware: native sessions where available)
services.AddHttpTransfers<MyTransferDelegate>();

// Linux / macOS / plain .NET (managed HttpClient loop)
services.AddHttpClientTransfers<MyTransferDelegate>();
