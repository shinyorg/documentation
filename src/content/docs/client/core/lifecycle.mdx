...
:::tip
Tag the handler with `[Singleton]` from `Shiny.Extensions.DependencyInjection` and call `services.AddGeneratedServices()` — the generator emits a registration that forwards to every interface the class implements (including any lifecycle hooks) in a single, AOT-clean call.
:::
...
## Foreground / Background (All Platforms)
...
:::tip
See [Startup Tasks](/client/core/startup) for more on `IShinyStartupTask` and how it relates to `ShinyLifecycleTask`.
:::
