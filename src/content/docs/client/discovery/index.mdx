...
<PlatformSupport frameworks={["maui", "dotnet"]} os={["android", "ios", "tvos", "windows", "linux", "macos"]} />
...
|-------------|----------|----------|------|
| Apple devices, AirPlay, IPP printers, peer apps, your own services | mDNS/DNS-SD | `AddMdns()` | [Browsing](/client/discovery/browsing/) · [Publishing](/client/discovery/publishing/) |
| Routers, UPnP/DLNA media servers, Sonos, Roku, smart TVs | SSDP/UPnP | `AddSsdp()` | [SSDP & UPnP](/client/discovery/ssdp/) |
| ONVIF IP cameras, WSD printers and scanners, Windows machines | WS-Discovery | `AddWsDiscovery()` | [WS-Discovery & ONVIF](/client/discovery/wsdiscovery/) |
...
- **No iOS multicast entitlement for mDNS** — Apple platforms go through Bonjour (`NSNetService`), not raw multicast sockets. SSDP and WS-Discovery *do* need it; see [Platform Setup](/client/discovery/platform/).
...
multicast entitlement on iOS and tvOS. See [Platform Setup](/client/discovery/platform/).

tvOS uses the same `NSNetService` implementation iOS does and needs the same
`NSBonjourServices` and `NSLocalNetworkUsageDescription` entries — an Apple TV is often the
device *doing* the discovering on a home network, so this is a first-class target rather than
a degraded one.
:::
...
- [Browsing & Resolving](/client/discovery/browsing/) — streaming vs one-shot, TXT records, resolve timeouts
- [Publishing](/client/discovery/publishing/) — TXT metadata, conflict renaming, lifetime
- [Platform Setup](/client/discovery/platform/) — Info.plist, manifest, firewall, and what backs each platform
