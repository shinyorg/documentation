...
selections on Android, a `CNContact` fetch predicate on iOS, Mac Catalyst and macOS — and are **always re-applied in-memory**,
...
| Field | Android | iOS / Mac Catalyst / macOS |
|-------|---------|-----|
...
more work. `CNContact`'s only search predicate matches on a **name-token prefix**, which is why the Apple
platforms push down `StartsWith`/`Equals` but not `Contains`/`EndsWith`, and never in `Match.Any` mode.
