...
<PlatformSupport frameworks={["dotnet", "maui", "blazor"]} os={["android", "ios", "tvos", "maccatalyst", "macos", "windows", "linux"]} />

`Shiny.Data.Sync` is a **reliable, background-capable, bidirectional JSON record-sync engine** between a mobile / desktop / web app and an HTTP backend. Where [`Shiny.Net.Http`](/client/httptransfers/) moves **files** in the background, `Shiny.Data.Sync` moves **records**: `Create` / `Update` / `Delete` operations queued in a local outbox and drained against a REST API, with a complementary inbox pulling deltas back from the server.
...
- **A scheduled job** — `AddDataSync` auto-registers a `SyncJob` with the [Shiny Jobs](/client/jobs/) scheduler so periodic pulls keep running while the app is backgrounded. No `AddJob` call required.
...
|---|---|---|---|
| iOS / Mac Catalyst / tvOS | Background `NSURLSession` upload task | Background `NSURLSession` download task | **Yes** (both directions) |
...
The right transport is picked automatically by `AddDataSync<TDelegate>` based on the target TFM. See [Architecture](/client/datasync/architecture) for why each tier looks the way it does, and [Platform Behavior](/client/datasync/platform-behavior) for the details and required Info.plist / manifest bits.
...
The four methods are the only opinionated contract the library imposes. See [Conflict Resolution](/client/datasync/conflict-resolution) for the merge story.
...
- [Architecture](/client/datasync/architecture) — why outbox + inbox separation, why per-endpoint cursors, why NSURLSession on Apple, why the Shiny.Json indirection
- [Entity Registration](/client/datasync/entity-registration) — every option on `SyncEndpoint`
- [Conflict Resolution](/client/datasync/conflict-resolution) — handling 409 / 412 with merges
- [Removal Strategies](/client/datasync/removal-strategies) — inline delete verbs, tombstones, soft-delete predicates, expiry predicates
- [Server API Contracts](/client/datasync/server-contracts) — exact JSON shapes the default `RestSyncTransport` speaks
- [Sync Interceptors](/client/datasync/sync-interceptor) — global auth, signing, tracing
- [Platform Behavior](/client/datasync/platform-behavior) — what runs where, and what survives app kill
- [Custom Transports](/client/datasync/custom-transports) — swap out the default REST transport
