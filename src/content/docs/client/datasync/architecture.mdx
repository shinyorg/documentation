...
The strategies *compose* — you can layer all three on one endpoint. See [Removal Strategies](/client/datasync/removal-strategies).
...
| Realtime push | Use [Shiny Push](/client/push/) or a SignalR connection to call `PullNow<T>` when the server has news. The engine doesn't open long-lived sockets. |
...
- **Large blobs.** Use [`Shiny.Net.Http`](/client/httptransfers/). Sync moves JSON records; transfers move multi-megabyte files with range-aware resume.
