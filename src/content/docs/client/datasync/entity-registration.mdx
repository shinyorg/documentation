...
builder.Services.AddDataSync<MyDataSyncDelegate>(opts =>
{
    ...
    opts.RegisterEndpoint<Project>("https://api.example.com/projects", ep =>
    {
        ...
        // null          = manual only: PullAll / SyncJob skip; only PullNow<T> pulls it
        // TimeSpan.Zero = always pull on every scheduled pass (the registration default)
        // positive      = throttle: PullAll / SyncJob skip while within the window; PullNow bypasses
...
`MinPullInterval` controls how an endpoint behaves on **scheduled** pulls (`PullAll`, the `SyncJob`, and connectivity-restore triggers). `PullNow<T>` always bypasses it.

| Value | Behavior |
|---|---|
| `TimeSpan.Zero` | **Always pull** on every scheduled pass. This is the default applied at registration. |
| A positive `TimeSpan` | **Throttle** — when `LastPulledAt` is within this window, `PullAll` / `SyncJob` skip the endpoint. |
| `null` | **Manual only** — scheduled pulls skip the endpoint entirely; it pulls only when you call `PullNow<T>`. |

:::note
Set `MinPullInterval = null` for endpoints you only ever want to refresh on demand (e.g. pull-to-refresh or a push-triggered `PullNow`), so the background `SyncJob` never touches them.
:::
...
See [Removal Strategies](/client/datasync/removal-strategies) for the full discussion.
...
| Property | Default | Description |
|---|---|---|
| `DefaultConflictPolicy` | `AskDelegate` | What to do when the server returns 409 / 412. See [Conflict Resolution](/client/datasync/conflict-resolution). |
...
See [Server API Contracts → Batched outbox](/client/datasync/server-contracts#batched-outbox) for the expected request and response shapes.
