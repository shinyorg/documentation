...
[`GetKnownNetworks()`](/client/wifi/known-networks/). Windows, macOS and Linux write an ordinary profile;
...
:::note[Addressing is available everywhere; the SSID is not]
IP, DNS, gateway and mask come from the managed network stack rather than from a Wi-Fi API, so they
need no permission and are populated on every platform. Individual fields are still best-effort:
a platform is allowed not to implement one — `GatewayAddresses` is unsupported on Android, and the
sandboxed platforms vary on the rest — and a field read that is refused comes back null (or an empty
list) rather than throwing, so one missing field never costs you the others. `Ssid` and `Bssid` are a
different problem: they need `WifiCapabilities.CurrentNetwork` and the platform permission behind it,
and come back null otherwise. That is why they are separate concerns.
:::
...
[Known Networks](/client/wifi/known-networks/) — including the important caveat that iOS and Android only
