...
If you've read [`Shiny.Jobs` architecture](/client/jobs/architecture), the playbook is similar — one DI registration, constraint-shaped requests, platform-tiered engines under a uniform surface.
...
The notification content is customisable via `IAndroidForegroundServiceDelegate` — see the [GPS page](/client/locations/gps#controlling-gps-notification-on-android).
...
- **You need exact-time geofence triggers (`"alert me at 6pm when I'm near home"`).** Combine with [Local Notifications](/client/notifications/) — geofencing tells you *where*, notifications tell you *when*.
...
- [`Shiny.Jobs` architecture](/client/jobs/architecture) — for deferred work that a geofence transition might trigger (sync, upload, notification).
- [Motion Activity](/client/locations/motionactivity) — pairs naturally with GPS for "stationary vs walking vs driving" inference.
