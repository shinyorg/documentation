---
title: Configuration
---

## MSBuild Configuration

| Property | Default | Description |
|---|---|---|
| `ShinyDIExtensionMethodName` | `AddGeneratedServices` | Name of the generated extension method |
| `ShinyDIExtensionNamespace` | Global namespace | Namespace for the generated code |
| `ShinyDIExtensionInternalAccessor` | `false` | Generate `internal` instead of `public` extension method |

```xml
<PropertyGroup>
    <ShinyDIExtensionMethodName>AddMyServices</ShinyDIExtensionMethodName>
    <ShinyDIExtensionNamespace>My.Namespace</ShinyDIExtensionNamespace>
    <ShinyDIExtensionInternalAccessor>true</ShinyDIExtensionInternalAccessor>
</PropertyGroup>
```

## AI Coding Assistant

An AI skill is available for Shiny DI to help generate attribute-driven service registrations, keyed services, and follow best practices directly in your IDE.

**Claude Code**
```bash
claude plugin add github:shinyorg/skills
```

**GitHub Copilot** — Copy the [shiny-di](https://github.com/shinyorg/skills/blob/main/skills/shiny-di/SKILL.md) skill file into your repository's [custom instructions](https://docs.github.com/en/copilot/customizing-copilot/adding-repository-custom-instructions).
