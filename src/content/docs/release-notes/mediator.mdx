...
# vNext - TBD
<RN type="enhancement">ASP.NET source generator now generates minimal API routes and the dependency injection mapping</RN>
<RN type="enhancement">ASP.NET source generator now places http attributes on the handler method, not the class</RN>
