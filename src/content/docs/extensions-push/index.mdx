...
Server-side push notification dispatch for .NET. A provider-agnostic core with three built-in transports —
direct **APNs** (token-based `.p8` / ES256 over HTTP/2, no FCM dependency for Apple), **FCM** (HTTP v1, with
multicast batching), and **Web Push** (VAPID + RFC 8291) — plus pluggable persistence (Shiny.DocumentDb),
structured targeting, an interceptor pipeline, automatic dead-token pruning, multi-app keyed registration,
and `System.Diagnostics.Metrics` telemetry. The whole library is AOT/trim-safe.
...
| | |
|---------|---------|
| Core (incl. APNs, FCM &amp; Web Push) | <NugetDownloads name="Shiny.Extensions.Push" /> |
...
- **Three transports in the box** — direct APNs (`.p8`/ES256 over HTTP/2), FCM (HTTP v1), and Web Push
  (VAPID + RFC 8291) all ship in the core package; each is opt-in via `AddApns` / `AddFcm` / `AddWebPush`
...
- **Rich payloads** — title/body, badge, sound, deep link, collapse id, TTL, silent/background pushes, plus
  per-platform escape hatches (`ApplePushOptions`, `AndroidPushOptions`, `WebPushOptions`)
...
Install the core package — it includes the APNs, FCM and Web Push transports — and, optionally, a
persistence package:
