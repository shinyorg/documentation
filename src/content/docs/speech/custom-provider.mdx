...
public class MyCloudTtsProvider : ITextToSpeechProvider
{
    ...
    // Optional: declare what you can do with a SpeechTone and with inline annotations.
    // Defaults to SpeechToneCapabilities.None, which strips annotations for you.
    ...
    public async Task<Stream> SynthesizeAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Applies the caller's AnnotationHandling, promotes inline tags like [excited] to a
        // tone, and strips them when ToneCapabilities says you can't perform them.
        var resolved = SpeechAnnotations.Resolve(text, options, this.ToneCapabilities);

        // Send resolved.Text (and resolved.Tone, if any) to your cloud API
...
`ToneCapabilities` is a default interface member, so an existing provider that doesn't declare it
keeps compiling and reports `None`. Calling `SpeechAnnotations.Resolve` is still worth doing in that
case — it strips bracketed annotations that would otherwise be read aloud. See
[Emotion & Tone](/speech/emotion) for the full contract.
