...
4. **The audio I/O abstractions are first-class, not internal helpers — and now ship separately.** `IAudioSource` and `IAudioPlayer` (plus `PipeStream` and `AccessState`) live in the standalone **`Shiny.Audio`** package/namespace. Cloud providers depend on them, the AI Conversation library depends on them, and apps that just need raw PCM or MP3 playback depend on them directly — some without needing the speech stack at all. `Shiny.Speech` references `Shiny.Audio`, so consumers add `using Shiny.Audio;` but get everything wired by `AddSpeechServices()`.
