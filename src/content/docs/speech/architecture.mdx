...
6. **Capability bits, not exceptions.** `IsSupported`, `IsListening`, `IsSpeaking`, `IsPlayerAnalysisSupported` let app code check what a platform can do without trying it and catching. The Browser doesn't have native TTS metering; Windows doesn't expose level taps; the API says so before you bind UI.
