...
Sort results at the SQL level using `OrderBy<T>`. Works with all `GetAll`, `Query`, `GetAllStream`, and `QueryStream` methods that accept expression predicates.

```csharp
// Ascending order
var youngest = await store.GetAll(ctx.User, OrderBy<User>.Ascending(u => u.Age));

// Descending order
var oldest = await store.GetAll(ctx.User, OrderBy<User>.Descending(u => u.Age));

// Combined with query predicate
var results = await store.Query<User>(
    u => u.Age > 25,
    ctx.User,
...
The `orderBy` parameter is optional (`null` by default) so all existing call sites continue to work without changes.
