...
| Downloads | <NugetDownloads name="Shiny.Extensions.Reflector" /> |
...
- Built-in `System.Text.Json` converter that serializes through the generated reflectors
- Opt-in fallback to true reflection for non-attributed types
...
`GetReflector()` — the parameterless overload — only ever returns a source-generated reflector, so it is trim
...
| `GetReflector(bool fallbackToTrueReflection)` | `[RequiresUnreferencedCode]` |
...
See [JSON Serialization](/reflector/json/) for the details on each.
