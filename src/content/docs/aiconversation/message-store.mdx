...
The `Store` method is called once after the AI finishes responding, with the triggering user message, the AI's display text, and the complete `ChatResponse`.

:::caution
Persist `assistantMessage`, **not** `response.Text`. With [structured turns](/aiconversation/structured-turns)
enabled (the default), `response.Text` is the raw JSON envelope — storing it would put JSON into your chat
history and feed it back into later prompts through the [chat lookup tool](/aiconversation/ai-tools).
`assistantMessage` is the parsed reply, or the raw text when the turn was unstructured. `response` is still
passed for metadata such as token usage.
:::
