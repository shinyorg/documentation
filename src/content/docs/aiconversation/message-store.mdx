...
The first `Store` overload receives the user's `ChatMessage` (from `Microsoft.Extensions.AI`). The second overload is called once after the AI finishes responding, with the complete `ChatResponse` and the triggering user message.
