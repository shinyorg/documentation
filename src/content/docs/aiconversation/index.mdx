...
- **Structured turns** — the AI answers with a typed `AiTurn` (`Reply` + `Questions`), so "keep listening" is a real signal rather than a guess at the wording; falls back to plain text on any provider that can't produce it
- **Conversation continuation** — while a turn carries questions the microphone stays open for the answer, bounded by `FollowUpTimeout`
- **Multiple choice** — questions can carry a fixed set of answers, rendered as tappable buttons by `AiChatView`
...
2. Register a chat client
   ...
   Building the UI in MAUI? `Shiny.AiConversation.Maui` gives you a ready-made chat screen — see [MAUI Chat UI](/aiconversation/chat-view).
...
If smooth, uninterrupted speech recognition is important for your app, consider using the [ElevenLabs](https://shinylib.net/speech/elevenlabs) cloud provider via [Shiny.Speech](https://shinylib.net/speech), which transcribes captured audio without relying on the native engine's start/stop behavior.

```csharp
builder.Services.AddElevenLabsSpeechToText("your-api-key");
```

**Avoid Azure on Android:** the [Azure AI Speech](https://shinylib.net/speech/azure) provider does **not** currently support Android's 16 KB memory page size. On devices that use 16 KB pages (Android 15+ on supported hardware), the Azure native libraries fail to load — prefer ElevenLabs on Android.
:::
...
* [Sample MAUI App](https://github.com/shinyorg/speech/tree/main/samples/MauiSample) — Full chat app with GitHub Copilot authentication, settings, and aura visualization
* [Sample Blazor WASM App](https://github.com/shinyorg/speech/tree/main/samples/BlazorSample) — Blazor WebAssembly app with static OpenAI provider
