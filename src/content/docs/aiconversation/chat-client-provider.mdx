...
Providers also declare how their endpoint can be asked for a [structured turn](/aiconversation/structured-turns).
`StructuredOutputMode` is a default interface member, so you only override it when the default is wrong
for your endpoint:
...
through per-model. A wrong value degrades rather than breaks — the service retries unstructured and falls
back to plain text.
