...
When using [table-per-type mapping](/data/sqlite-docdb/#table-per-type-mapping), you can designate a different property as the Id via `MapTypeToTable<T>("table", x => x.MyProperty)`. The same type constraints (`Guid`, `int`, `long`, `string`) and auto-generation rules apply to the custom property.
:::
...
`SetProperty` updates a single scalar field in-place using SQLite's `json_set()` — no deserialization, no full document replacement. Returns `true` if the document was found and updated, `false` if not found. The `id` parameter accepts `Guid`, `int`, `long`, or `string`.
...
`RemoveProperty` strips a field from the stored JSON using SQLite's `json_remove()`. Returns `true` if the document was found and updated, `false` if not found. The removed field will have its C# default value on next read. The `id` parameter accepts `Guid`, `int`, `long`, or `string`.
...
See [Querying](/data/sqlite-docdb/querying) for more examples of bulk delete and update with expressions.
