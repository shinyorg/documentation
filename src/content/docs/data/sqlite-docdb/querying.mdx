...
```
:::note
The table name `documents` is the default. When using [table-per-type mapping](/data/sqlite-docdb/#table-per-type-mapping), the query targets the mapped table and the `TypeName` filter is omitted for dedicated tables.
:::
