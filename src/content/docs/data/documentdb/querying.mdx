...
```
:::note
The table name `documents` is the default. When using [table-per-type mapping](/data/documentdb/#table-per-type-mapping), the query targets the mapped table and the `TypeName` filter is omitted for dedicated tables. The generated SQL varies by provider — each provider uses its own JSON extraction syntax.
:::
