...
Every one of those registrations **composes**: call `AddCors` twice, or `AddAuthorization` from the
app and again from a feature, and both calls' configuration applies, in the order they were made. A
package that adds a registration of its own should do the same with `OptionsRegistration.Configure`:
