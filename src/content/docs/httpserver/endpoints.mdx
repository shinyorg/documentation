...
<NugetBadge name="Shiny.Net.HttpServer" showLabel={true} />

```bash
dotnet add package Shiny.Net.HttpServer
```

There is nothing else to add. The generator ships inside the core package under `analyzers/`, next to
...
Each also has an overload on `IEndpointRouteBuilder`, so a class can be mounted under a
[group's prefix](/httpserver/routing/#groups) or from inside a module:

```csharp
app.MapGroup("/v2", api => api.MapWidgetEndpoints());   // [Route("/api/widgets")] → /v2/api/widgets
```
