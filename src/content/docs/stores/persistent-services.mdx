...
The static accessor is initialized automatically by the hosted initializer that `AddShinyStores()` registers. For host-less scenarios (unit tests, console apps without `IHost`), call `Shiny.Stores.Initialize(serviceProvider)` manually after building your provider.

:::caution
`Shiny.Stores.Default`/`Secure`/`Keyed` throw `InvalidOperationException` if you call them before initialization. Make sure `AddShinyStores()` is called and either the host has started (so the hosted initializer fires) or `Stores.Initialize(...)` has been called explicitly.
:::
