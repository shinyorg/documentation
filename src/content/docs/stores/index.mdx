...
The accessor is wired up automatically by `AddShinyStores()` via a hosted initializer. For host-less tests/console apps, call `Shiny.Stores.Initialize(serviceProvider)` after `BuildServiceProvider()`.
