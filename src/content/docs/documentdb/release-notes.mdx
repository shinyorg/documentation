...
<RN type="feature">
**Full geometry spatial queries** — the spatial feature grows from point-only to full OGC geometry. A new `Geometry` model (`GeoLineString`, `GeoPolygon` with holes, `GeoMultiPoint`, `GeoMultiLineString`, `GeoMultiPolygon`, `GeoGeometryCollection`; `GeoPoint` implicitly converts to a point geometry) serializes as GeoJSON in the document body, and `MapSpatialProperty<T>(x => x.Area)` now accepts a `Geometry?` property alongside the existing `GeoPoint?` overload. Query it with the `Geo`-prefixed topological predicate family — `GeoIntersects`, `GeoContainedBy`, `GeoContains`, `GeoDisjoint`, `GeoTouches`, `GeoCrosses`, `GeoOverlaps`, `GeoEquals`, `GeoCovers`, `GeoCoveredBy` — plus `GeoWithinDistance(geometry, meters)` for a distance band. Every predicate takes an optional `orderByDistanceFrom` (point **or** geometry) and returns `SpatialResult<T>` with `DistanceMeters` populated; `NearestNeighbors` now works over geometry-mapped types too. The `Geometry` model also exposes in-memory `Area`/`Length`/`Perimeter`/`Centroid`/`NumPoints`/`NumGeometries` (compose with a materialized `MapComputedProperty` for indexed `area > X` queries) and `IsValid`/`IsSimple`/`MakeValid`. **Provider tiers:** **SQLite** runs the full family via the R\*Tree bbox prune + an in-process relate/refine (no native dependency); **CosmosDB** pushes `ST_INTERSECTS`/`ST_WITHIN`/`ST_DISTANCE` down natively (finer predicates refine over the intersect candidate set); **MongoDB** gains spatial support via a `2dsphere` index with native `$geoIntersects`/`$geoWithin`/`$near` (finer predicates and non-point distance refine in-process). `GeoDisjoint` is anti-selective and scans the type (O(n)) on the two-pass providers. Other providers remain `SupportsSpatial => false`. Existing `GeoPoint` mappings and `WithinRadius`/`WithinBoundingBox`/`NearestNeighbors` are unchanged. See [Spatial](/documentdb/spatial/).
</RN>
