...
<RN type="breaking">
**Interceptor cleanup — scoped interceptors just work, no marker.** Building on the session model, the interceptor scope machinery is gone: **`IScopedDocumentInterceptor` is removed** (drop the marker — a plain `IDocumentInterceptor` now gets `ctx.Services`), and the **scoped-interceptor ban is lifted** — `services.AddScoped<IDocumentInterceptor, X>()` is supported. Interceptors are resolved **fresh from the flowing scope per write** (a scoped session's own request scope, or a per-unit fallback child scope for scope-less immediate writes), so scoped services resolve the caller's own instances with no captive-singleton problem. `ctx.Services` is now **non-nullable**. New: **`ctx.Session`** — an `IDocumentSession` bound to the current write's transaction, the natural handle for re-entrant side effects (`ctx.Session.Add(outbox); await ctx.Session.SaveChanges();` flushes into the same transaction); `ctx.Store` still works. **Migration:** change `: IScopedDocumentInterceptor` to `: IDocumentInterceptor`; drop any `ctx.Services == null` checks; remove `AddDocumentStoreInstrumentation`-style scoped-interceptor workarounds.
</RN>
...
<RN type="breaking">
**Telemetry is now embedded and always-on.** The instrumentation decorator (`InstrumentedDocumentStore`), the `AddDocumentStoreInstrumentation()` extension methods, and the `DocumentStoreOptions.Instrumentation` flag were **removed**. Every store now emits OpenTelemetry metrics + trace spans directly, on every provider and every construction path — including a plain `new …DocumentStore(options)`, which the decorator could never reach. It stays zero-cost when unobserved (`ActivitySource.StartActivity` returns null with no listener; the `Meter` instruments no-op with no subscriber). **Migration:** delete any `AddDocumentStoreInstrumentation()` calls and `o.Instrumentation = true`; just subscribe your OTel pipeline to `.AddMeter("Shiny.DocumentDb")` / `.AddSource("Shiny.DocumentDb")`. Operation names and `db.*` tags are unchanged, so existing dashboards keep working — with one refinement: a `UnitOfWork` emits a single `transaction` span (its inner writes run span-free) rather than a child span per buffered op.
</RN>

<RN type="breaking">
**DI registration and telemetry folded into the core package.** `Shiny.DocumentDb.Extensions.DependencyInjection` (`AddDocumentStore`, `AddDocumentContext`, seeding, multi-tenancy) and `Shiny.DocumentDb.Diagnostics` (`AddDocumentStoreInstrumentation`, OpenTelemetry metrics + tracing) now ship **inside the core `Shiny.DocumentDb` package**. The types keep their `Shiny.DocumentDb` namespace, so code is source-compatible — just **remove the two now-obsolete `PackageReference`s**; your `using Shiny.DocumentDb;` and every `AddDocumentStore` / `AddDocumentStoreInstrumentation` call keep working. Core gains three lightweight, AOT-safe dependencies (`Microsoft.Extensions.DependencyInjection.Abstractions`, `Microsoft.Extensions.Hosting.Abstractions`, `Microsoft.Extensions.Diagnostics`). A bonus: `options.Instrumentation = true` now auto-wires the decorator with no extra package.
</RN>

<RN type="feature">
**Scoped DI + transaction-visible store for write interceptors.** A write-time interceptor can now resolve **request-scoped** services from `ctx.Services` while staying a singleton — implement the `IScopedDocumentInterceptor` marker and read scoped deps inside the hook, not the constructor. Through a scoped [`DocumentContext`](/documentdb/context) it's the caller's own request scope; through a raw singleton store (MAUI, background worker, Orleans grain) the pipeline opens one fresh child scope per write and disposes it after. The hooks also get `ctx.Store`, a **transaction-bound** store handle: a single write with per-doc interceptors runs as an implicit one-op unit of work, so a hook can read this unit's uncommitted rows and write side effects (an outbox row) that commit **atomically** with the triggering write — with no shared-connection deadlock. Add `Order` to sequence interceptors deterministically. This also fixes a latent bug: DI-registered interceptors now fire on **every** provider (they previously silently never ran on MongoDB, Cosmos, LiteDB, IndexedDB, DynamoDB, Azure Table — and in Orleans grain storage). Full transaction visibility is a relational + LiteDB guarantee; other backends follow their own model (see [Interceptors](/documentdb/interceptors)).
</RN>
