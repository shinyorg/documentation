...
<RN type="enhancement">
**Both admin front ends now carry the Shiny mark.** In the web app it sits in the brand bar and links to
[shinylib.net](https://shinylib.net); in the terminal app it is drawn on startup, as the logo itself in
half-block cells beside the version and the site. The terminal banner goes on the normal screen rather than
the app's, so it scrolls into your shell history instead of costing a frame, and it is skipped when output
is redirected, when the terminal has no colour, when `NO_COLOR` is set, or with `--no-splash`.
</RN>
