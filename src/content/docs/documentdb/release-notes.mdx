...
<RN type="feature">
**Spatial predicates in LINQ via `DocumentFunctions`** — compose a spatial predicate with the rest of a query (other `Where` clauses, `OrderBy`, `Count`, paging), all server-side: `store.Query<Zone>().Where(z => DocumentFunctions.Intersects(z.Area!, area) && z.Active).OrderBy(z => DocumentFunctions.Distance(z.Area!, origin))`. The family — `Intersects`, `Disjoint`, `Contains`, `Within`, `Covers`, `CoveredBy`, `Touches`, `Crosses`, `Overlaps`, `GeoEquals`, `WithinDistance` (in `Where`) and `Distance` (in `OrderBy`) — lowers to each engine's native spatial function or a registered UDF. **SQLite** runs an R\*Tree bbox prune + `docdb_st_*` C# UDF refine (indexed, dependency-free); **MySQL, DuckDB, and PostgreSQL** compute native `ST_*` on the fly from the JSON body pruned by the envelope index (PostgreSQL needs the PostGIS extension; DuckDB auto-loads `spatial`). Set `PortableSpatial = true` on a relational provider to force the dependency-free envelope tier. On **SQL Server, Oracle, CosmosDB, and MongoDB**, a `DocumentFunctions` spatial call in a `Where` currently throws a clear message — use the dedicated `store.Geo*` methods there. See [Spatial](/documentdb/spatial/#linq-composition-with-documentfunctions).
</RN>
