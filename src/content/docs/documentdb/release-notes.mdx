...
<RN type="enhancement">
**Typed-context generator is now bundled in `Shiny.DocumentDb`** — the `DocumentContext` / `DocumentSet<T>` source generator that shipped in 10.0 as a separate **`Shiny.DocumentDb.Generators`** package is now packaged **inside the core `Shiny.DocumentDb` package** (under `analyzers/dotnet/cs`). Installing `Shiny.DocumentDb` is all you need — the generator, the `[Document]` attribute, and the `DocumentContext`/`DocumentSet<T>` runtime types all arrive together and stay version-locked, matching how `System.Text.Json`, `Microsoft.Extensions.Logging`, and the MVVM Toolkit ship their generators. Because it's a build-time analyzer (a development dependency), it adds **no** runtime or AOT footprint to the core package. **Upgrading from 10.0.0:** if you added a `<PackageReference Include="Shiny.DocumentDb.Generators" />` explicitly, **remove it** — leaving it in place runs the generator twice and produces duplicate-member compile errors. The standalone `Shiny.DocumentDb.Generators` package is no longer published. See [Typed Context](/documentdb/context/).
</RN>
