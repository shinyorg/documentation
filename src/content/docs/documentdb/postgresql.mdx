...
- **Native spatial needs PostGIS** — the provider wires it through in full (a `geometry` sidecar column, a
  GiST index and `ST_*` pushdown), but the stock `postgres` image doesn't ship the extension. Use a
  PostGIS-enabled image (`postgis/postgis`), grant the connecting role permission to `CREATE EXTENSION`, or
  set [`PortableSpatial = true`](/documentdb/spatial#portable-vs-native-spatial) for the dependency-free
  envelope tier.
