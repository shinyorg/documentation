...
:::tip[Created / updated timestamps]
The store already records when each document was created and last written. Declare a `DocumentMetadata` property
to see those values on the document and query on them — see [Document Metadata](/documentdb/metadata).
:::

:::tip[Custom Id property]
Designate a different property as the Id with `cfg.MapIdProperty(x => x.MyProperty)` inside a `ConfigureDocument<T>` block — on its own (the type stays in the default shared table) or alongside `cfg.Table` for a dedicated one. The same type constraints (`Guid`, `int`, `long`, `string`) and auto-generation rules apply to the custom property.
:::
...
- **Sidecars follow the same rule.** Blobs, the spatial index and the vector index are stored outside the document body, and a merge only touches the ones the patch actually carries. A patch that omits a mapped blob, geometry or embedding leaves the stored one — and its sidecar row — untouched, so a partial patch never silently removes a document from spatial or vector search. To genuinely clear one, use a write that carries the whole document (`Update`, or `Upsert(patchIfUpdate: false)`) with the property set to `null`.
