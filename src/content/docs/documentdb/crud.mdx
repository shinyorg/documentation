...
:::caution[Typed-object patch is coarse — prefer the JSON lane for precise partial updates]
A typed object always serializes **every** property, so `patch: true` only skips the fields that are `null` — a non-nullable field with a default (e.g. `int Count` = `0`) is still written. For a true "change only these keys" update, either make the patch type's fields nullable (`JsonIgnoreCondition.WhenWritingNull`/`WhenWritingDefault`), or use the late-bound **JSON lane**, where you control exactly which keys are present:

```csharp
// Only 'name' is present → only 'name' changes; every other field is untouched.
await store.Update(typeof(User), new JsonObject { ["id"] = "user-1", ["name"] = "Bob" }, patch: true);
await store.Upsert(typeof(User), partialJson, patchIfUpdate: false);   // replace-or-insert
```
:::

:::note[Provider tier]
The merge/replace **flags** are implemented on the relational providers (SQLite, SQLCipher, MySQL, SQL Server, PostgreSQL, Oracle, DuckDB) and the JSON lane. The two default behaviours — `Update` (replace) and `Upsert` (merge) — work on every store; the non-default modes (`Update(patch: true)`, `Upsert(patchIfUpdate: false)`) throw `NotSupportedException` on the document-native and key-partitioned stores.
:::
...
This lane **rides the full normal write pipeline** — tenancy, temporal history, versioning/optimistic-CAS, spatial + vector sidecars, JSON interceptors, and change notifications all apply (unlike [`IDocumentBackup`/`BulkImport`](/documentdb/backup/), which deliberately bypasses them). The generated Id (and bumped version) is written back onto your node, exactly like the typed `Insert<T>`.
