...
2. Register with dependency injection
   ...
   {
       cfg.ToCollection("orders");
       cfg.MapVersionProperty(o => o.RowVersion);
   });
...
By default every document type shares the `"documents"` collection (or whatever `CollectionName` is set to). Use `cfg.ToCollection()` to give a type its own dedicated collection. Two types cannot map to the same collection name — registration throws `ArgumentException`.
...
options.ConfigureDocument<User>(cfg => cfg.ToCollection());           // auto-derived ("User")
...
options.ConfigureDocument<Tenant>(cfg =>                              // auto-derived + custom Id
{
    cfg.ToCollection();
    cfg.MapIdProperty(t => t.TenantCode);
});
```
...
| Member | Description |
|---|---|
| `cfg.ToCollection()` | Auto-derive the collection name from the type name |
| `cfg.ToCollection("orders")` | Explicit collection name |
| `cfg.Table = "orders"` | The provider-agnostic spelling of the same thing |
| `cfg.MapIdProperty(x => x.MyKey)` | Custom Id, independent of the collection mapping |
...
Predicates are translated into a MongoDB `FilterDefinition`. Field comparisons, `Any`/`Count`, and `In` use native filter operators; scalar functions (`ToLower`, `Length`, `Substring`, `Math.*`, date parts, …) translate to an `$expr` aggregation, and flag-enum `HasFlag` uses `$bitsAllSet`. For the full grammar see [Querying](/documentdb/querying/) and [Provider Reference](/documentdb/providers/).
...
uow.Add(new Order { Id = "ord-1", Status = "Pending" })
   .Add(new Payment { Id = "pay-1", OrderId = "ord-1" });
