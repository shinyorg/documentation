...
`ExportAsync` writes the entire store out to a `Stream` as a **backup document** — a JSON array of `{ "id", "docType", "data", "createdAt", "updatedAt" }` records, where `data` is the raw document body emitted **as-is**. Sidecar tables (temporal history, spatial, vector, full-text indexes) are *not* exported; they are rebuilt by the write path on restore.

:::note[Timestamps are preserved]
The export records each document's `createdAt` / `updatedAt`, and an `Insert`-mode restore binds them per row, so a backup round trip preserves the original creation and modification times across every provider (relational + MongoDB/CosmosDB) rather than re-stamping them to the import time. The format is backward-compatible: an older v1 backup (no timestamps) still imports, stamping the current time.
:::
...
- **The outbox table is carried by export, import and `ClearAll`** like any other document table, which is mostly what you want — a backup that includes in-flight messages is a real recovery aid. Because [the outbox lives in its own table](/documentdb/outbox#storage) by default, a caller who wants business data without infrastructure messages excludes one table rather than filtering rows out of a shared one. Restoring an outbox restores *undelivered* messages too: they will be delivered again once a processor starts.
...
- [Transactional outbox](/documentdb/outbox) — what the `outbox` table in an export is.
