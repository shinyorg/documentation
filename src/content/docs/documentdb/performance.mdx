...
Serialization runs on every read and write, so it sits on the hot path. Prefer a source-generated
`JsonSerializerContext` (or, on the strongly-typed `DocumentContext`, `[Document(..., JsonContext = typeof(...))]`)
