...
A `UnitOfWork` commits once. A loop of `Insert` calls outside a transaction commits once per call. Fifty inserts wrapped in a transaction is roughly 50× faster than fifty unwrapped inserts on SQLite. Use `BatchInsert` when possible, a `UnitOfWork` when you need mixed operations.
