...
`ExecuteUpdate` only handles one property per call. If you need to set several, prefer `Update` of the full document — or call `ExecuteUpdate` in sequence.

## Serialization mode

Serialization runs on every read and write, so it sits on the hot path. Prefer source-generated metadata —
a `JsonSerializerContext`, or on the strongly-typed `DocumentContext` either `[Document(..., JsonContext = typeof(...))]`
or `[Document(..., Serialization = DocumentSerialization.Generated)]` (the generator emits the metadata for you) —
over reflection. `DocumentSerialization.Reflection` is the convenient escape hatch for non-AOT apps, but it
is slower per row and not trimming-safe. See [AOT Setup](/documentdb/aot/) for the full mode comparison.
...
Use `cfg.Table` for any type that:
- Will accumulate large row counts, or
- You query frequently, or
- Has indexes you want to keep narrow.

```csharp
opts.ConfigureDocument<Order>(cfg => cfg.Table = "orders");           // hot, large
opts.ConfigureDocument<AuditLog>(cfg => cfg.Table = "audit_logs");   // append-heavy
...
A session commits once. A loop of `Insert` calls outside a transaction commits once per call. Fifty inserts wrapped in a transaction is roughly 50× faster than fifty unwrapped inserts on SQLite. Use `BatchInsert` when possible, a session when you need mixed operations.

```csharp
await using var session = store.OpenSession();
session.Add(parent)
   .AddRange(children);   // coalesced into one batch insert
