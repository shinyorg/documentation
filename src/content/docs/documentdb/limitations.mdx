...
- **Streaming methods hold the internal semaphore** for the duration of enumeration. Do not interleave other store operations inside the same `await foreach`.

## Indexing

- `CreateIndexAsync` only exists on the **concrete SQL `DocumentStore` types** (SQLite, SQLCipher, PostgreSQL, SQL Server, MySQL). It is not on `IDocumentStore`.
...
- **CosmosDB, LiteDB, IndexedDB** do not expose `CreateIndexAsync`. CosmosDB indexes everything automatically (tune via container indexing policy). LiteDB does not configure indexes. IndexedDB indexes are fixed at object-store creation time (when you bump `Version`).
...
- **SQLite** is reader-many / writer-one. Two simultaneous writers serialize.
- **LiteDB** is single-process: opening the same file from two processes fails.
- **IndexedDB** is single-tab effectively — multi-tab writes can interleave non-deterministically.
- The other server providers (Postgres / SQL Server / MySQL / Cosmos) support concurrent writers per their native semantics.
