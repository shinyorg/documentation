...
| **Oracle** | Requires Oracle Database **23ai+**. No spatial. No vector search. No native change feed. No `Backup()`. `SetProperty`/`RemoveProperty` go through PL/SQL helper functions (the connecting user needs `CREATE PROCEDURE`). |
