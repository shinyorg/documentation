...
```csharp
opts.ConfigureDocument<Invoice>(cfg => cfg.MapBlob(i => i.Signature, o =>
{
...
:::caution[Clearing a blob needs a replace, not a merge]
`Upsert` is an RFC 7396 merge and strips nulls from the patch, so `invoice.Pdf = null` reads as *"this patch doesn't
mention Pdf"* — the stored blob and its sidecar row survive. That is what makes partial patches safe, but it means a
merge can never drop a blob. To actually remove one, use a write that carries the whole document:

```csharp
invoice.Pdf = null;
await store.Update(invoice);                      // or: store.Upsert(invoice, patchIfUpdate: false)
```

Removing an item from a `DocumentBlobCollection` works on either path — the collection itself is present in the
patch, so the merge sees the shorter list.
:::
