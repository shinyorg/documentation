...
| **Spatial / geo queries** | Yes — full OGC geometry (`MapSpatialProperty`), topological predicates + composable `DocumentFunctions` in LINQ, native 2-D spatial index on every SQL provider (R\*Tree, GiST, `SPATIAL`, `SDO_GEOMETRY`) plus Cosmos `ST_*` / Mongo `2dsphere` | No | Provider-specific (NetTopologySuite) |
