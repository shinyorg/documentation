...
| **Transactions** | `store.OpenSession()` + `SaveChanges()` | `RunInTransactionAsync` | `SaveChanges` / explicit transactions |
| **Vector / ANN search** | Yes — cross-provider (pgvector, sqlite-vec, DiskANN, …) | No | Provider-specific only |
| **Full-text search** | Yes — `cfg.MapFullTextProperty` + relevance-ranked search on every provider (FTS5, `tsvector`, Oracle Text, …) | No | Provider-specific only |
| **Spatial / geo queries** | Yes — full OGC geometry (`cfg.MapSpatialProperty`), topological predicates + composable `DocumentFunctions` in LINQ, native 2-D spatial index on every SQL provider (R\*Tree, GiST, `SPATIAL`, `SDO_GEOMETRY`) plus Cosmos `ST_*` / Mongo `2dsphere` | No | Provider-specific (NetTopologySuite) |
| **Temporal history** | Yes — `cfg.MapTemporal` (history / as-of / diff / restore), all providers | No | SQL Server temporal tables only |
...
**The batteries are included.** Full-text search, vector/ANN search, spatial/geo queries (full OGC geometry over a native 2-D spatial index), temporal history, multi-tenancy, global query filters, change feeds, offline-first HTTP sync, OData endpoints, LLM tool functions, JSON Schema validation, and OpenTelemetry are all first-party and work across the provider set — not features you bolt on with a second library and reconcile against your storage layer. With sqlite-net or EF Core, each of those is a separate integration you own.
...
- **Choose Shiny.DocumentDb** when your data is document-shaped (nested objects, child collections, variable structure), when you target .NET MAUI / AOT, when you want one storage API across a dozen databases, or when you want vector, full-text, and spatial/geo search, temporal history, multi-tenancy, change feeds, offline sync, or built-in telemetry without bolting on more libraries.
