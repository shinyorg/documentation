...
```
:::note
The table name `documents` is the default. When using [table-per-type mapping](/documentdb/#table-per-type-mapping), aggregates target the mapped table automatically. The generated SQL varies by provider — each provider uses its own JSON extraction syntax.
:::
