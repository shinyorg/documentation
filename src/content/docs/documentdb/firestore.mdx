...
:::note[Running on iOS/Android?]
This package uses the **admin** SDK and a service account, which bypasses security rules — it is for server/backend use, and a service-account key must never ship inside an app. For a store that runs **on the device** under the end user's Firebase identity, with the native offline cache, use [Firestore Mobile](/documentdb/firestore-mobile/).
:::
...
Mapping methods: `cfg.ToCollection`, `cfg.MapIdProperty`/`MapIdType`, `cfg.MapVersionProperty`, `cfg.AddQueryFilter`, and the interceptor hooks.
