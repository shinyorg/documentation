...
Cosmos transactions are scoped to a single partition key (`/typeName`). The provider applies a `UnitOfWork` using a **compensating model**: inserts in the unit are tracked and deleted on failure. Updates and removes in the unit are not compensated.
