...
2. Dependency injection
   ...
   options.ConfigureDocument<User>(cfg => cfg.ToContainer());
   options.ConfigureDocument<Order>(cfg => cfg.ToContainer("orders"));
...
options.ConfigureDocument<User>(cfg => cfg.ToContainer());                  // auto-derived name
options.ConfigureDocument<Order>(cfg => cfg.ToContainer("orders"));        // explicit name
options.ConfigureDocument<Sensor>(cfg =>                                   // explicit name + custom Id
{
    cfg.ToContainer("sensors");
...
opts.ConfigureDocument<Restaurant>(cfg => cfg.MapSpatialProperty(r => r.Location));
...
Cosmos transactions are scoped to a single partition key (`/typeName`). The provider applies writes (a session) using a **compensating model**: inserts in the unit are tracked and deleted on failure. Updates and removes in the unit are not compensated.

For Cosmos's native `TransactionalBatch`, the provider chunks `BatchInsert<T>` into 100-item batches (the Cosmos limit).

`BatchUpsert<T>`, `BatchUpdate<T>`, and `BatchRemove<T>` — along with `Clear<T>()` and `Query<T>().ExecuteDelete()` — issue their per-document requests in bounded-concurrency waves rather than one at a time. Cosmos has no cross-document transaction for heterogeneous operations, so these batches are best-effort (not atomic), consistent with the compensating unit-of-work model above; the win is parallelized round-trips (and therefore RU spend).

## Temporal History

Cosmos DB supports [temporal history](/documentdb/temporal/) — `MapTemporal<T>` records every version to a `{container}_history` sidecar container (partitioned by `/typeName`, like the main container), read back via the `ITemporalDocumentStore` capability interface (`History`/`AsOf`/`Restore`/`GetDiffBetween`/`AsOfAll`/…). Resolve or cast to `ITemporalDocumentStore`; the methods are not on the base `IDocumentStore`. The history container is auto-provisioned at the configured throughput on first use.
...
- **Document storage / migration** — the document body is stored as a nested JSON object under `data`, so server-side `Where` filtering, scalar functions, and flag-enum queries work. Documents written by versions **before 7.2** stored `data` as an escaped JSON string and are **not matched by server-side filters** until re-saved (`Update`/`Upsert`); reads still work in the meantime.
- Optimistic concurrency works via `cfg.MapVersionProperty` on `CosmosDbDocumentStoreOptions`.
