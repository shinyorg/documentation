...
If you provide your own `sqlite-vec` binary, statically link it and register via `sqlite3_auto_extension(sqlite3_vec_init)` at startup, then set `SqliteDatabaseProvider.VectorExtensionPreloaded = true` to skip the runtime load. `VectorExtensionPreloaded` makes `SupportsVector` return `true`; if both it and `EnableVectorExtension` are set, the preloaded path wins. On desktop/server you can instead keep `EnableVectorExtension` with a shipped binary.
:::
