...
> **Scoped `update` requires an `Id` property.** The update path re-fetches the stored record by its `Id` to verify scope. Types whose identifier is mapped to a differently-named property (via `MapIdProperty`) can't be re-fetched this way, so a scoped `update` on such a type throws. `Get`/`Delete`/`Insert`/`Query` are unaffected.
