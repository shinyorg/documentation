...
| Capability | SQLite | SQLCipher | PostgreSQL | SQL Server | MySQL | DuckDB | CosmosDB | MongoDB | LiteDB | IndexedDB |
|---|---|---|---|---|---|---|---|---|---|---|
...
| `CreateIndexAsync<T>(x => x.Prop)` | yes | yes | yes | **emits broken DDL — see below** | yes | yes (functional index on `json_extract_string`) | n/a (Cosmos auto-indexes) | n/a (Mongo indexes manually) | n/a | n/a (schema-time only) |
| Composite / multi-column index | no | no | no | no | no | no | n/a | n/a | n/a | n/a |
