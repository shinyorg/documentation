...
| Predicate translated to native query | pushdown on promoted columns (`MapIndexedProperty`); residue in C# | pushdown (`FilterExpression`) on promoted attributes; residue in C# |
...
| Optimistic concurrency (`MapVersionProperty`) | yes — native `ETag` If-Match | yes — conditional write on a `Version` attribute |
