...
| CockroachDB | [CockroachDB](/documentdb/cockroachdb/) |
| SQL Server | [SQL Server](/documentdb/sqlserver/) |
| MySQL | [MySQL](/documentdb/mysql/) |
| MariaDB | [MariaDB](/documentdb/mariadb/) |
| Oracle | [Oracle](/documentdb/oracle/) |
| DuckDB | [DuckDB](/documentdb/duckdb/) |
| Azure Cosmos DB | [Cosmos DB](/documentdb/cosmosdb/) |
| Azure Table Storage | [Azure Table](/documentdb/azure-table/) |
| Amazon DynamoDB | [DynamoDB](/documentdb/dynamodb/) |
| MongoDB | [MongoDB](/documentdb/mongodb/) |
| Amazon DocumentDB | [Amazon DocumentDB](/documentdb/amazon-documentdb/) |
| Redis (Redis Stack) | [Redis](/documentdb/redis/) |
| RavenDB | [RavenDB](/documentdb/ravendb/) |
| Google Firestore | [Firestore](/documentdb/firestore/) |
| Google Firestore (on-device, iOS/Android) | [Firestore Mobile](/documentdb/firestore-mobile/) |
...
## Key-partitioned NoSQL providers (Azure Table & DynamoDB)

[Azure Table Storage](/documentdb/azure-table/) and [Amazon DynamoDB](/documentdb/dynamodb/) are schema-free **key-partitioned** stores. They map the library's `(typeName, id)` identity onto the store's native partition/sort keys (`PartitionKey/RowKey` on Table, `pk/sk` HASH/RANGE on DynamoDB) and behave like LiteDB — queries evaluate **client-side** — but against a managed cloud backend.

| Capability | Azure Table | DynamoDB |
|---|---|---|
| Storage | `Data` string column in one table, `PartitionKey=typeName` / `RowKey=id` | `Data` string attribute in one table, `pk=typeName` / `sk=id` |
| `Query<T>(string sql, ...)` raw query | yes — **OData** `$filter` over promoted columns | yes — **PartiQL** over promoted attributes |
| Predicate translated to native query | pushdown on promoted columns (`cfg.MapIndexedProperty`); residue in C# | pushdown (`FilterExpression`) on promoted attributes; residue in C# |
| Promoted/indexed columns | `MapIndexedProperty<T>` → native column + OData `$filter` | `MapIndexedProperty<T>` → native attribute + `FilterExpression` |
| `Upsert` deep merge (RFC 7396) | yes (C# recursive merge) | yes (C# recursive merge) |
| Optimistic concurrency (`cfg.MapVersionProperty`) | yes — native `ETag` If-Match | yes — conditional write on a `Version` attribute |
| Int/Long Id auto-generation | **no** (use Guid/string) | **no** (use Guid/string) |
| Spatial / vector / full-text / temporal | no | no |
| In-process observation (`IObservableDocumentStore`) | yes | yes |
| Native change feed (`IChangeFeedDocumentStore`) | no | **yes — DynamoDB Streams** |
| Transactions | compensating (track inserts, undo on failure) | compensating |
| `BatchInsert` chunk size | 100 per PartitionKey (`SubmitTransaction`) | 25 per request (`BatchWriteItem`) |
| Size cap | ~64 KB per property / 1 MB per entity | 400 KB per item |
| `ClearAll` (`IDocumentMaintenance`) | yes | yes |

Because `Query<T>()` is always type-scoped, the common read path is a single-partition query on both. Loading the whole partition before filtering is a **full type scan** unless the filtered property is promoted with `MapIndexedProperty<T>` (which pushes the filter server-side) — so promote hot query paths or keep per-type document counts bounded.
...
A session uses a **compensating model** rather than a multi-document session: inserts are tracked and deleted on failure, matching the CosmosDB provider. To use true ACID transactions, run MongoDB in a replica set and lift the constraint in your fork. Updates and removes in a unit are **not** compensated.
...
SQLite quotes table identifiers with `"` so a type named `Order` or `User` works as a mapped table. The other providers follow their native quoting rules; if you hit a reserved-word collision, prefer an explicit non-reserved name via `cfg.Table = "orders"`.
...
The document providers in this repo share one implementation of `IDocumentQuery<T>`, and an out-of-repo provider can use it too. Derive from **`DocumentQueryBase<T>`** and supply four members — everything else (builder state and immutability, query-filter resolution, the client-side terminals, grouping, cursor pagination, string projection, and the whole set-based-write interceptor dance) is inherited:

```csharp
public class MyDocumentQuery<T> : DocumentQueryBase<T> where T : class
{
    readonly MyStore store;

    internal MyDocumentQuery(MyStore store, JsonTypeInfo<T>? typeInfo)
        : base(store.BuildQueryContext(typeInfo)) => this.store = store;

    MyDocumentQuery(MyDocumentQuery<T> source) : base(source) => this.store = source.store;

    protected override DocumentQueryBase<T> Clone() => new MyDocumentQuery<T>(this);

    protected override async Task<QueryExecution<T>> ExecuteAsync(QueryPlan<T> plan, CancellationToken ct)
        // Narrow however your engine can, then say what you satisfied:
        //   Candidates(rows)  – nothing applied; the base filters, orders and pages
        //   Filtered(rows)    – predicates applied server-side, ordering/paging not
        //   Complete(rows)    – predicates, ordering and paging all applied server-side
        => QueryExecution<T>.Candidates(await this.store.LoadAllAsync(this.Context.TypeName, ct));

    protected override Task<int> DeleteMatchingAsync(QueryPlan<T> plan, CancellationToken ct)
        => this.store.DeleteWhereAsync(plan.CompilePredicate(), ct);

    protected override Task<int> SetPropertyMatchingAsync(QueryPlan<T> plan, string jsonPath, object? value, CancellationToken ct)
        => this.store.SetPropertyWhereAsync(plan.CompilePredicate(), jsonPath, value, ct);
}
```

:::caution[Report push-down honestly]
`QueryExecution<T>` is a contract, not a hint. Claiming ordering you did not apply returns rows in the wrong order; claiming paging you did not apply drops rows. When in doubt return `Candidates` — correct and slower beats fast and wrong.
:::

Optional hooks (`ObserveChanges`, `FullTextSearchCore`, `NearestVectorsCore`, `ToQueryString`, `ToCursorPage`) throw `NotSupportedException` until you override them, and the aggregate hooks (`CountCore`, `MaxCore`, …) default to materializing — override them only where your engine computes the aggregate itself.

`SetPropertiesMatchingAsync` (the multi-property `ExecuteUpdate`) also has a working default — it applies the assignments one at a time through `SetPropertyMatchingAsync`. Override it when your engine can set several JSON paths in one statement, so the update stays atomic and the predicate is evaluated once.

On the store side, derive from `DocumentProviderBase` and implement `Mappings`, `IdCache`, `ResolveTypeInfo`, and `ResolveDocumentTypeName`; your write methods then bracket their persistence with the shared pipeline:

```csharp
async Task InsertImpl<T>(T document, JsonTypeInfo<T>? jsonTypeInfo, CancellationToken ct) where T : class
{
    var write = await this.BeginWriteAsync(DocumentOperation.Insert, document, null, jsonTypeInfo, ct);
    if (!write.Proceed)                 // an interceptor replaced this write with its own
        return;
    document = write.Doc;               // …and may have swapped the document

    var id = this.ResolveInsertId(write, accessor => this.GenerateId(accessor));
    write.VersionMapping?.SetVersion(document, 1);

    // …your persistence here…

    await this.CompleteWriteAsync(write, id, 1, DocumentChangeType.Inserted, document, ct);
}
```

`CompleteWriteAsync` runs the `AfterWrite` chain and publishes the change (buffered until commit when a unit of work is open). `RequireDocumentId(write)` is the update-side counterpart of `ResolveInsertId`.

The `DocumentQueryContext<T>` your store builds carries the type name, telemetry tracker, interceptor pipeline, serializer options, query filters, computed-property applier, and id accessor. On the options side, implement **`IDocumentStoreOptions`** (three explicitly-implemented members) and every cross-cutting feature — soft delete, JSON Schema validation — works on your provider with no further code.
