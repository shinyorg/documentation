...
2. Dependency injection
   ...
   builder.Services.AddDynamoDbDocumentStore(o =>
   {
       ...
       o.ConfigureDocument<Order>(cfg => cfg.MapVersionProperty(x => x.Version));   // opt-in optimistic concurrency
   });
   ```
...
```csharp
var options = new DynamoDbDocumentStoreOptions { TableName = "Documents", Region = RegionEndpoint.USEast1 };
...
- **Optimistic concurrency:** `cfg.MapVersionProperty(x => x.Version)` seeds the version to 1 on insert (stored as a top-level `Version` number attribute), checks & increments on update/upsert, and guards the write with `ConditionExpression: Version = :expected`. A stale write throws `ConcurrencyException`. Blind (unversioned) upsert is last-write-wins. Insert uses `attribute_not_exists(sk)` so a duplicate id throws.
