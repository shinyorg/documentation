...
- **Composite JSON indexes** — `CreateIndexAsync(ctx.User, u => u.Country, u => u.Age)` builds a single B-tree across multiple JSON paths on SQLite, SQLCipher, PostgreSQL, MySQL, DuckDB, and SQL Server. [Learn more](/documentdb/indexes/#composite--multi-column-indexes)
