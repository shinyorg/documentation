...
Additional per-type mappings, all inside a `ConfigureDocument<T>` block: `cfg.ToPartition(pk)`, `cfg.MapIdProperty(...)`, `cfg.AddQueryFilter(...)`, `cfg.OnBeforeWrite` / `cfg.OnAfterWrite`, and `cfg.MapVersionProperty(...)`. Store-level: `options.MapIdType<TId>(...)` and `options.AddInterceptor(...)`.
...
```csharp
var options = new AzureTableDocumentStoreOptions { ConnectionString = "…", TableName = "Documents" };
options.ConfigureDocument<Order>(cfg =>
{
    cfg.MapIndexedProperty(o => o.Status);
    cfg.MapIndexedProperty(o => o.Total);
});
...
Notifications are in-process (this instance's writes), buffered in a session until it commits. For a cross-writer feed use DynamoDB (Streams) or Cosmos.
...
- **Optimistic concurrency:** `cfg.MapVersionProperty(x => x.Version)` seeds the version to 1 on insert, checks & increments on update/upsert, and guards the write with the Table `ETag` (If-Match). A stale write throws `ConcurrencyException`. Blind (unversioned) upsert is last-write-wins.
