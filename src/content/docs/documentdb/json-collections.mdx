...
var users = store.Collection<User>();          // or store.Collection(typeof(User))
...
- **`Collection(type)`** / **`Collection<T>()`** — you *have* a document type but not an instance: HTTP
  intake, message-bus payloads, gateways, anything that already holds JSON and shouldn't pay to
  deserialize it. Writes as well as reads, and the query is the string grammar.
- **`Query<T>()` with a [JSON terminal](/documentdb/querying/#raw-json-results)** — you want the *typed*
  builder (`Where(o => …)`, LINQ ordering, spatial, `hasflag` without quoting) but a raw JSON **result**:
  `ToJsonList()`, `FirstOrDefaultRawJson()`, `WriteJsonArrayTo(stream)`. Read-only, and it works on every
  provider rather than just the ones below.
- **`Query<T>()`** with the normal terminals — you have CLR instances. Nothing here replaces it.
:::
...
var first  = await orders.Query().OrderBy("created desc").FirstOrDefault();   // one row, not the whole set
var only   = await orders.Query().Where($"id == {id}").Single();              // throws if a second matches
...
// Several fields in one statement
await orders.Query()
    .Where("status == 'draft'")
