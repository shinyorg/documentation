...
A **computed property** is a value *derived* from other fields — `Total = Quantity * UnitPrice`, `FullName = First + " " + Last`, a normalized `lower(Email)` — that you can **filter, sort, and project by exactly like a stored property**, even though it is never written into the document JSON. Map it once with `cfg.MapComputedProperty(...)` and reference it by name in typed LINQ, the string query API, and OData.
...
```csharp
opts.ConfigureDocument<Order>(cfg => cfg.MapComputedProperty<decimal>(o => o.Total, o => o.Quantity * o.UnitPrice,
    indexed: true));
```
...
```csharp
opts.ConfigureDocument<Order>(cfg => cfg.MapComputedProperty<decimal>(
    "Total",
