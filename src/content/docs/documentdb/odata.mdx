...
:::tip[REST or OData?]
`Shiny.DocumentDb.OData` is the right choice when the client already speaks OData — `$filter`, `$metadata`, an
existing toolchain. When you own both ends, [REST & live-query endpoints](/documentdb/rest-endpoints) give you
plain JSON, writes, Server-Sent-Events and an AOT-clean package with no third-party dependency. Never map both
on the same route prefix.
:::
...
Any `cfg.AddQueryFilter` predicates (multi-tenant / soft-delete / "active only") run **beneath** the
...
A public OData endpoint should never be wide open. Each entity set carries an `ODataQueryPolicy`:
...
:::note[Status codes]
`400` — a well-formed request that the policy disallows, or a query naming an unknown property. `501` —
`$expand` (documents have no relationships) or a spatial filter on a non-spatial provider. `200` —
everything else (the page size is silently clamped to the configured cap).
:::

### Encrypted properties are served, not hidden

An entity set over a type with [encrypted properties](/documentdb/encryption/) returns their **decrypted**
values, exactly as `Get<T>()` does — field-level encryption protects data *at rest*, and an OData response is a
materialized document like any other. The policy allowlists above gate `$filter`/`$orderby`/`$select`; they do
not remove a property from the response.

If a property must not leave the process, don't put its type on the wire: project to a DTO and map an entity
set over that instead.
...
`samples/Sample.ODataApi` in the repo is a minimal ASP.NET Core app that wires this up against a
SQLite store: it registers a seeder (`AddDocumentSeeder`) that fills the store once at startup with
~250 **customers** and ~1,000 **orders** generated by [Bogus](https://github.com/bchristie/Bogus)
...
caps + per-property allowlists) — see [Governance](#governance--caps-allowlists--complexity-limits).
