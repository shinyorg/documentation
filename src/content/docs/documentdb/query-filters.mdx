...
For the soft-delete case you don't have to wire the filter (or the write side) yourself — `AddSoftDelete<T>(x => x.IsDeleted)` registers the filter *and* turns `Remove`/`ExecuteDelete`/`Clear` into flag updates. See [Soft Delete](/documentdb/soft-delete).
:::
