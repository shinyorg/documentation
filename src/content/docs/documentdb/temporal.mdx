...
options.ConfigureDocument<Order>(cfg => cfg.MapTemporal(o =>
{
    ...
    o.ResolveActor = sp => sp.GetService<ICurrentUser>()?.Id;  // scope-aware (11.0): resolve the actor from
                                                     // the write's session DI scope (a request-scoped user);
                                                     // takes precedence over CaptureActor.
}));
```

Every `Insert`, `Update`, `Upsert`, `Remove`, `SetProperty`, `RemoveProperty`, and `BatchInsert` for `Order` now appends a version — including writes made in a session (buffered and committed atomically). Only mapped types incur the extra write; everything else is untouched.
...
`Restore` writes a **new** current version (it does not rewrite history): the document is re-inserted if it had been removed, otherwise overwritten with the version-7 state. Returns `null` if that version doesn't exist or was a removal tombstone. When a [version property](/documentdb/crud/) is mapped and the live row still exists, the optimistic-concurrency token is aligned to the live row so the restore isn't rejected as stale, then bumps forward like a normal update.

:::note[Restoring a removed document starts a fresh lifecycle]
If the live document had been **removed**, restore re-creates it as a new live row: its `CreatedAt` is stamped to the restore time and a mapped version counter restarts at 1. The append-only history — including the original creation and the removal tombstone — is preserved and continues its own monotonic version sequence. This "undelete = re-create" contract is uniform across all temporal providers (relational + LiteDB/Mongo/Cosmos/IndexedDB).
:::
...
Because temporal adds new object stores, IndexedDB only creates them during a schema upgrade. A **fresh** database picks them up automatically; for an **already-deployed** one, increment `options.Version` when you add `cfg.MapTemporal` so the upgrade runs and creates the `{store}_history` object stores.

## Limitations

- **Set-based writes do not record per-document history** — `Clear<T>`, `ExecuteDelete` and `ExecuteUpdate` never materialize the documents they affect, so there is nothing to snapshot. Use `Remove<T>` / `Update<T>` per document when you need the change tracked. This applies to set-based writes made by the library's own features and by the [admin tool](/documentdb/admin/) alike.
