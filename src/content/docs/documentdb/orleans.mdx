...
The generic escape hatch — you build a fully-configured `IDocumentStore` with `GrainStateRecord` mapped (type→table/collection/container **and** `cfg.MapVersionProperty`). Use this for LiteDB, IndexedDB, or any other backend.
...
o.StoreFactory = sp =>
{
    ...
    DocumentDbGrainStorage.ConfigureGrainState(opts, "orleans_default");
    opts.ConfigureDocument<GrainStateRecord>(cfg => cfg.MapTemporal(t => t.MaxVersions = 100));  // audit trail
...
`AddDocumentDbClustering(...)` registers cluster membership. Default table `orleans_membership`. The per-silo rows and a single global table-version row are updated **together in a single session (one transaction)**, each gated on its own version (CAS) — this is how Orleans' table-version protocol is honored.
...
### Streams (`IQueueAdapterFactory`)

`AddDocumentDbStreams("Default", ...)` registers a **persistent stream provider** — durable, inspectable Orleans streams on the same store. Default table `orleans_streams`. Unlike the stores above it runs only on backends with real row-level locking (PostgreSQL, SQL Server, MySQL, MariaDB, Oracle, CockroachDB), because a locked per-queue counter row is what keeps stream sequencing correct under concurrent producers.

See **[Orleans Streams](/documentdb/orleans-streams)** for the full picture — throughput expectations, the sequencing design, idle-poll cost, and delivery guarantees.
