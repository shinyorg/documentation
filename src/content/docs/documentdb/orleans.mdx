...
- **Reflection-based serialization** — grain state and system-store documents are (de)serialized via reflective `System.Text.Json`; this is a server-side provider and is not an AOT/trim target like the rest of the stack.
