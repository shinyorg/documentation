...
<LibBuilder componentName="documentdb-indexeddb" client:load />
...
1. Register with dependency injection in your Blazor WASM `Program.cs`:
   ...
   options.ConfigureDocument<User>(cfg => cfg.ToStore());          // dedicated object store
   options.ConfigureDocument<Order>(cfg => cfg.ToStore("orders")); // explicit store name
...
options.ConfigureDocument<User>(cfg => cfg.ToStore());            // auto-derived store name
options.ConfigureDocument<Order>(cfg => cfg.ToStore("orders"));   // explicit store name
options.ConfigureDocument<Sensor>(cfg =>                          // custom Id property
{
    cfg.ToStore();
...
- `Count<T>()`
- `store.OpenSession()` + `SaveChanges()` (operations are batched in a single IndexedDB transaction)
...
opts.ConfigureDocument<NewType>(cfg => cfg.ToStore(cfg.TypeName));
```
...
IndexedDB supports [temporal history](/documentdb/temporal/) — `MapTemporal<T>` records every version to a `{store}_history` object store, read back via the `ITemporalDocumentStore` capability interface (`History`/`AsOf`/`Restore`/`GetDiffBetween`/`AsOfAll`/…). Resolve or cast to `ITemporalDocumentStore`; the methods are not on the base `IDocumentStore`.

Because temporal adds new object stores, **bump `Version`** when adding `cfg.MapTemporal` to an already-deployed database so the upgrade creates the history stores (a fresh database needs no change):

```csharp
opts.Version = 2;            // was 1 — schema upgrade creates the {store}_history object stores
opts.ConfigureDocument<Order>(cfg => cfg.MapTemporal());
```
