...
1. Install the package
   ...
   :::caution[Do not update SQLitePCLRaw to 3.x]
   SQLCipher needs the SQLitePCLRaw **2.1.x** graph. SQLitePCLRaw 3.x ships no SQLCipher bundle, and an app gets exactly one SQLitePCLRaw core. Adding any SQLitePCLRaw 3.x package (for example to silence the `NU1903` restore warning) therefore **breaks SQLCipher for now**: encrypted opens fail with *"the native SQLite library 'e_sqlite3' doesn't support encryption"*. See [SQLitePCLRaw version and the security advisory](/documentdb/sqlite/#sqlitepclraw-version-and-the-security-advisory) for what the warning means and why it isn't a critical problem for mobile apps. The SQLCipher 4.5.2 build in `bundle_e_sqlcipher` 2.1.11 is itself based on SQLite 3.39.2, so it predates that fix either way.
   :::

   For **vector search** on encrypted databases, also add `Shiny.DocumentDb.Sqlite.VectorSupport` and call `SqliteVec.RegisterAutoExtension()` at startup — it registers `vec0` against the active `e_sqlcipher` engine — then set `VectorExtensionPreloaded = true` on your `SqlCipherDatabaseProvider`. See [Vector search](/documentdb/vector/#sqlite--sqlite-vec-virtual-table).
...
- Use **[field-level encryption](/documentdb/encryption/)** (in the core package) when specific *properties* must be ciphertext — on any provider, not just SQLite. It is complementary: SQLCipher protects the whole file from someone who has the file, field-level encryption protects named values from anyone who can read the database (including a DBA, a replica, or a backup). Use both if you want both.
