...
| `DocumentSerialization.Generated` | (planned) | Reserved for a future generator mode that emits the metadata for you from the `[Document]` declaration alone. Until it ships, use `JsonContext`. |
...
:::caution
The generated `JsonTypeInfo<T>` must be **metadata-mode** (populated `.Properties`) — the query translator
reads property metadata to map CLR members to JSON paths. `JsonContext` mode satisfies this because
System.Text.Json's source generator produces metadata-mode type info. `Reflection` works too but isn't
trimming-safe; a value-converter wrapper would have empty `.Properties` and break property-based queries.
:::
