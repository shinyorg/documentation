...
`FullTextSearch`/`FullTextMatch` are *terminal* ranked calls. When you instead want full-text as a **composable predicate** — AND/OR'd with ordinary filters, paged, and sorted alongside everything else — use `DocumentFunctions.LuceneMatch(field, luceneQuery)` in a `Where` and `DocumentFunctions.LuceneScore(field, luceneQuery)` in an `OrderBy`/projection. They translate a Lucene query string to the provider's native full-text engine over the **same `MapFullTextProperty` index** (so the type must still be mapped). The `field` argument identifies the mapping — pass a mapped property; the search spans the type's whole combined index.
