...
`Instrumentation = true` is honored by the **non-keyed** `AddDocumentStore` overloads (including the multi-tenant one). Setting it on a keyed/named store throws `NotSupportedException`, because the decorator only wraps the non-keyed registration — instrument those manually if you need them. You still point your OpenTelemetry pipeline at the `Shiny.DocumentDb` meter/source as shown above.
