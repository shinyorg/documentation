...
- Optional parameters — those with an explicit default (e.g. `IFoo? foo = null`) or a nullable reference annotation — resolve via `GetService<T>()` (or `GetKeyedService<T>("key")`) so an unregistered dependency does **not** throw; the declared default is used as the fallback:

```csharp
[Singleton]
public class MyService(IFoo? foo = null) { }

// Generated:
// services.AddSingleton<MyService>(sp => new MyService(sp.GetService<global::Foo>()));
```
