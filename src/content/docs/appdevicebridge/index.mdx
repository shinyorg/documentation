...
Coming from Blazor Hybrid? See [AppDeviceBridge vs. Blazor Hybrid](/appdevicebridge/vs-blazor-hybrid/) for how the
two models differ and when to pick each.
...
| --- | --- | --- |
| `Shiny.AppDeviceBridge.Maui` | the app | `UseAppDeviceBridge(bridge => …)`: `AddShinyHttpServer` with the bridges on it, started with the app and restarted on resume; `MauiAppDeviceBridgeBuilder`, which the MAUI bridges extend; it also calls `UseShiny()` and supplies the app's UI thread; `UseTrafficMonitor` and `TrafficMonitorPage` for debugging — see [Traffic monitor](/appdevicebridge/hosting/#traffic-monitor) |
...
| `Shiny.AppDeviceBridge.Simulator` | a .NET tool | `shiny-bridge-sim`: a terminal UI (or, with `--web`, a browser panel) that serves every bridge with answers you set, events you fire and trails you play (GPX walks included), with a traffic monitor, and an MCP endpoint an AI agent can drive it through — test a page without a device. See [Simulator](/appdevicebridge/simulator/) |
...
| `Shiny.AppDeviceBridge.Wearables` | the app | `AddWearablesBridge()`: the companion Apple Watch or Wear OS app, through Shiny.Wearables — status, live messages the page or `background.js` answers, shared context, queued transfers and files through the file roots (iOS and Android; `501` elsewhere). See [Wearables](/appdevicebridge/wearables/) |
| `Shiny.AppDeviceBridge.Maps` | the app | `AddMapsBridge()`: vector map tiles online or from regions the user downloads (verified against a signed catalog), and turn-by-turn directions from an online Valhalla router (all platforms). See [Maps & Directions](/appdevicebridge/maps/) |
