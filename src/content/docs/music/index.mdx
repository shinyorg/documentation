...
* Manage custom playlists — create, remove, and add tracks via `IMusicManager`
* Track play counts per song via `IMusicManager`
* SQLite-backed storage via `Shiny.Music.Sqlite` with AOT-compatible serialization
...
| Custom Playlists (IMusicManager) | ✅ (SQLite) | ✅ (SQLite) |
| Play Count Tracking | ✅ (SQLite) | ✅ (SQLite) |
