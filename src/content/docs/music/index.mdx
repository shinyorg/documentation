...
* Play Apple Music tracks on iOS
* Check for active streaming subscriptions
* Identify songs by listening to audio through the microphone (iOS via ShazamKit)
...
* Manage playlists — create, remove, and add/remove tracks via `IMediaLibrary`
* Automatic play count tracking (iOS via MPMediaItem, Android via local storage)
* Play counts available on `MusicMetadata.PlayCount`
...
| Streaming Playback | ❌ | ✅ (MPMusicPlayerController) |
| Lyrics (Plain + Synced LRC) | ✅ (LRCLIB) | ✅ (LRCLIB) |
| Album Art | ✅ (MediaStore) | ✅ (MPMediaItem Artwork) |
| Copy Track | ✅ | ✅ (non-DRM only) |
| Streaming Subscription Check | ❌ (always `false`) | ✅ (MusicKit) |
| Song Identification | ❌ | ✅ (ShazamKit) |
| Explicit Flag | ❌ | ✅ (MPMediaItem.IsExplicitItem) |
| Playlist CRUD | ✅ (JSON-backed custom playlists) | ✅ (JSON-backed custom playlists) |
| Play Count Tracking | ✅ (local JSON store) | ✅ (MPMediaItem.PlayCount) |
...
public class MyPage
{
    ...
    async Task PlayFirstTrack()
    {
        ...
        // 5. Fetch lyrics (plain or synced LRC)
