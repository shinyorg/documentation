...
* Detect the [audio output route](/music/output-devices) with `IAudioOutputDevices` — speaker, wired/USB, Bluetooth, car, HDMI, or AirPlay — with `IsWired()` / `IsBluetooth()` / `IsHeadphones()` helpers and a `Changed` event for pausing when headphones are unplugged
...
| Output route + change events | ✅ (all connected outputs; current is derived) | ✅ (`AVAudioSession.CurrentRoute`; active route only) |
