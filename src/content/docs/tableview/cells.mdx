...
:::note
When a property is set to `null` or `-1`, it falls back to the global value set on the parent `TableView`. This is the cascading style system — see [Styling](/tableview/styling) for details.
:::
